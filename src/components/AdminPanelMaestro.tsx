      ...
      {adminTab === 'explosivos' && (
        <div className="bg-white border-4 border-slate-900 rounded-[3rem] p-10 shadow-2xl space-y-10">
           ...
           <div className="grid md:grid-cols-2 gap-8 text-slate-900">
              <div className="space-y-4"><label className="font-black text-xs uppercase">3 Animales Regalo</label><Input className="border-4 border-slate-900 h-14 rounded-xl font-black" placeholder="Ej: 12, 05, 31" /></div>
              <div className="space-y-4"><label className="font-black text-xs uppercase">Cargar Pirámide</label><div className="border-4 border-dashed border-slate-100 p-8 rounded-2xl flex items-center justify-center cursor-pointer hover:bg-slate-50"><ImageIcon className="text-slate-200" size={40} /></div></div>
           </div>
           <Button className="w-full h-16 bg-emerald-500 rounded-2xl font-black uppercase text-white shadow-xl">Guardar Todo</Button>
        </div>
      )}
    </div>
  );
