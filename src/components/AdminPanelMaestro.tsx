...
import { Store, Key, Upload, Loader2, CheckCircle2 } from "lucide-center"; // Asegúrate de que las rutas sean correctas en tu proyecto
import { Store as StoreIcon, Key as KeyIcon, Upload as UploadIcon, Loader2 as LoaderIcon, CheckCircle2 as CheckIcon } from "lucide-react";
...
export function AdminPanelMaestro({ userRole }: { userRole?: string }) {
  ...
  const handleSaveAgencia = async () => {
    if (!nombre || !pagoMovil || !cedula || !banco) {
      return toast.error("Chamo, faltan datos obligatorios");
    }
    ...
    try {
      ...
      if (imageFile) {
        ...
        const fileName = `publicidad/${Date.now()}.${fileExt}`;
      ...
      // 2. Guardar en la tabla 'agencies'
  ...
  if (!auth) {
    return (
      <div className="p-10 flex flex-col items-center bg-white border-4 border-slate-900 rounded-[4rem] shadow-2xl mx-auto max-w-2xl mt-10">
        <KeyIcon size={60} className="text-emerald-600 mb-6" />
      ...
      {adminTab === 'agencias' && (
        <div className="bg-white border-4 border-slate-900 rounded-[3rem] p-6 md:p-10 shadow-2xl space-y-10 animate-in slide-in-from-bottom-4">
           <h3 className="font-black text-2xl md:text-3xl uppercase italic text-pink-600 flex items-center gap-3 border-b-4 border-slate-100 pb-4">
             <StoreIcon size={32} /> GESTIÓN AGENCIAS
           </h3>

           <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div className="space-y-2">
                <label className="font-black text-xs uppercase ml-2 text-slate-500">Nombre de la Agencia</label>
              ...
              <div className="space-y-2">
                <label className="font-black text-xs uppercase ml-2 text-slate-500">Número de Pago Móvil</label>
              ...
              <div className="space-y-2">
                <label className="font-black text-xs uppercase ml-2 text-slate-500">Cédula / RIF</label>
              ...
              <div className="space-y-2">
                <label className="font-black text-xs uppercase ml-2 text-slate-500">Banco</label>
                <Input value={banco} onChange={e => setBanco(e.target.value)} className="border-4 border-slate-900 h-16 rounded-2xl font-black text-slate-900 text-lg" placeholder="Nombre del Banco" />
              </div>

              <div className="space-y-3 md:col-span-2">
                <label className="font-black text-sm uppercase ml-2 text-slate-600">Cargar Imagen Publicitaria</label>
                   ...
                   <div className={`absolute inset-0 border-4 border-dashed border-slate-900 rounded-3xl flex flex-col items-center justify-center bg-slate-50 transition-all ${previewUrl ? 'border-emerald-500 bg-emerald-50' : ''}`}>
                      {previewUrl ? (
                        <div className="flex items-center gap-6 p-4">
                          <img src={previewUrl} className="h-32 w-48 object-cover rounded-xl border-2 border-slate-900 shadow-xl" alt="Preview" />
                          <div className="flex flex-col">
                            <span className="text-emerald-600 font-black text-xs flex items-center gap-1 uppercase"><CheckIcon size={14}/> Imagen Lista</span>
                            <span className="text-slate-400 text-[10px] uppercase font-bold">Toca para cambiar</span>
                          </div>
                        </div>
                      ) : (
                        <>
                          <UploadIcon className="text-slate-400 mb-2" size={40} />
                          <span className="font-black text-slate-400 uppercase text-xs">Subir Publicidad</span>
                        </>
                      )}
                   </div>
                </div>
           ...
           >
             {loading ? <><LoaderIcon className="animate-spin mr-2" /> GUARDANDO...</> : "GUARDAR DATOS AGENCIA"}
           </Button>
        </div>
      )}
    </div>
