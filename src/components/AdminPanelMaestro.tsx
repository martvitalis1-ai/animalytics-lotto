      ...
      <div className="flex flex-wrap justify-center gap-1.5 bg-white p-3 rounded-[3rem] border-4 border-slate-900 shadow-xl max-w-4xl mx-auto">
        {['resultados', 'accesos', 'regalos', 'agencias'].map((tab: any) => (
          <button 
            ...
            className={`flex-1 min-w-[100px] px-4 py-3 rounded-full font-black text-[10px] md:text-xs uppercase transition-all ${adminTab === tab ? 'bg-slate-900 text-white shadow-md' : 'text-slate-400 hover:bg-slate-50'}`}
          >
            {tab}
          </button>
        ))}
      ...
      {adminTab === 'agencias' && (
        <div className="bg-white border-4 border-slate-900 rounded-[3rem] p-8 shadow-2xl space-y-8 animate-in slide-in-from-bottom-4">
           ...
           <div className="grid md:grid-cols-2 gap-6">
              <div className="space-y-2"><label className="font-black text-xs uppercase ml-2">Nombre Agencia</label><Input className="border-4 border-slate-900 h-14 rounded-2xl font-black" placeholder="Nombre" /></div>
              <div className="space-y-2"><label className="font-black text-xs uppercase ml-2">Pago Móvil</label><Input className="border-4 border-slate-900 h-14 rounded-2xl font-black" placeholder="Teléfono" /></div>
              <div className="space-y-2 md:col-span-2"><label className="font-black text-xs uppercase ml-2">Banco</label><Input className="border-4 border-slate-900 h-14 rounded-2xl font-black" placeholder="Banco / Cédula / Tipo" /></div>
              <div className="space-y-2 md:col-span-2"><label className="font-black text-xs uppercase ml-2">Banner Publicitario</label><Input className="border-4 border-slate-900 h-14 rounded-2xl font-black" placeholder="URL imagen" /></div>
           </div>
           <Button className="w-full h-16 bg-emerald-500 rounded-2xl font-black uppercase text-white shadow-xl border-b-8 border-emerald-700">Guardar</Button>
        </div>
      )}
    </div>
  );
