...
import { Save, Upload, Instagram, Loader2, Image as ImageIcon } from "lucide-react";
            ...
            <div className="grid lg:grid-cols-2 gap-10">
              <div className="space-y-6">
                <Input 
                  ...
                  placeholder="Nombre" className="h-14 rounded-2xl font-black uppercase bg-slate-100 border-none" 
                />
                <Input 
                  value={ag.whatsapp} 
                  onChange={e => setAgencias(agencias.map(x => x.id === ag.id ? {...x, whatsapp: e.target.value} : x))} 
                  placeholder="WhatsApp" className="h-14 rounded-2xl font-black bg-slate-100 border-none" 
                />
                <div className="relative">
                  ...
                  <Input 
                    ...
                    placeholder="URL Instagram" className="h-14 rounded-2xl pl-12 font-bold bg-slate-100 border-none" 
                  />
                </div>
                <div className="p-6 bg-slate-900 rounded-[2rem] space-y-3">
                  ...
                  <Input 
                    ...
                    placeholder="Banco" className="bg-white/10 border-none text-white h-12" 
                  />
                  <div className="grid grid-cols-2 gap-2">
                    <Input 
                      value={ag.banco_telefono || ""} 
                      onChange={e => setAgencias(agencias.map(x => x.id === ag.id ? {...x, banco_telefono: e.target.value} : x))} 
                      placeholder="Tlf" className="bg-white/10 border-none text-white h-12" 
                    />
                    <Input 
                      value={ag.banco_cedula || ""} 
                      onChange={e => setAgencias(agencias.map(x => x.id === ag.id ? {...x, banco_cedula: e.target.value} : x))} 
                      placeholder="Cédula" className="bg-white/10 border-none text-white h-12" 
                    />
                  </div>
                </div>
              </div>
              <div className="space-y-6">
                <p className="text-[10px] font-black uppercase text-slate-400">Publicidad (Banner Final)</p>
                ...
                <Button onClick={() => updateAgencia(ag)} className="w-full h-16 bg-emerald-600 hover:bg-emerald-700 text-white font-black uppercase rounded-2xl text-lg shadow-xl">
