...
export function AdminAgencias() {
  ...
  if (loading) return <div className="p-20 text-center font-black uppercase">Cargando Búnker Admin...</div>;

  return (
    <div className="max-w-7xl mx-auto p-4 lg:p-10 space-y-12 bg-slate-50 min-h-screen text-slate-900">
      <h1 className="text-4xl font-black uppercase italic border-b-8 border-emerald-500 pb-2">Gestión de Agencias</h1>
      
      <div className="grid gap-10">
        {agencias.map((ag) => (
          <Card key={ag.id} className="p-8 lg:p-12 rounded-[3rem] shadow-2xl border-none bg-white relative overflow-hidden text-left">
            <div className="grid lg:grid-cols-3 gap-12">
              
              {/* COLUMNA 1: IDENTIDAD */}
              <div className="space-y-6">
                <p className="text-[11px] font-black uppercase text-emerald-500 tracking-widest italic">1. Identidad y Logo</p>
                <div className="flex flex-col items-center p-6 bg-slate-50 rounded-[2.5rem] gap-4 border-2 border-slate-100">
                   ...
                   <span className="text-[10px] font-black text-slate-400">TOCA EL CÍRCULO PARA SUBIR LOGO</span>
                </div>
                <Input value={ag.nombre} onChange={e => setAgencias(agencias.map(x => x.id === ag.id ? {...x, nombre: e.target.value} : x))} placeholder="Nombre" className="h-14 rounded-2xl font-black uppercase bg-slate-100 border-none px-6" />
                <Input value={ag.whatsapp} onChange={e => setAgencias(agencias.map(x => x.id === ag.id ? {...x, whatsapp: e.target.value} : x))} placeholder="WhatsApp" className="h-14 rounded-2xl font-black bg-slate-100 border-none px-6" />
              ...
              {/* COLUMNA 2: PAGO MÓVIL */}
              <div className="space-y-6">
                <p className="text-[11px] font-black uppercase text-blue-500 tracking-widest italic">2. Datos de Pago</p>
                <div className="p-8 bg-slate-900 rounded-[3rem] space-y-4 shadow-2xl">
                  ...
                  <Input value={ag.banco_telefono || ""} onChange={e => setAgencias(agencias.map(x => x.id === ag.id ? {...x, banco_telefono: e.target.value} : x))} placeholder="Teléfono" className="bg-white/10 border-none text-white h-12 rounded-xl" />
                ...
                <Button onClick={() => saveAgencia(ag)} className="w-full h-20 bg-emerald-600 hover:bg-emerald-700 text-white font-black uppercase rounded-[2rem] text-xl shadow-xl transition-all">
              ...
              <div className="space-y-6">
                <p className="text-[11px] font-black uppercase text-amber-500 tracking-widest italic">3. Publicidad</p>
                <div className="relative h-72 border-4 border-dashed border-slate-200 rounded-[3rem] flex items-center justify-center bg-slate-50 overflow-hidden group">
                ...
                <p className="text-[9px] text-center font-black uppercase text-slate-400">SUBIR BANNER PUBLICITARIO (GALERÍA)</p>
              </div>
