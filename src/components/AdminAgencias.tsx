...
export function AdminAgencias() {
  ...
  const handleSave = async () => {
    if (!nombre || !whatsapp || !banco) return toast.error("Faltan datos obligatorios");
    setLoading(true);
    try {
      let publicUrl = null;
      ...
      toast.error("Error: Verifica que el bucket 'logos_agencias' sea PUBLICO en Supabase Storage");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <Card className="glass-card border-2 border-primary/20">
      <CardHeader><CardTitle className="text-sm font-black uppercase italic flex items-center gap-2"><Store className="w-5 h-5 text-primary" /> Configurar Agencias y Logos</CardTitle></CardHeader>
      <CardContent className="space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-left">
          <div className="space-y-3">
            <p className="text-[10px] font-black text-primary uppercase border-b pb-1">1. Datos y Logo</p>
            ...
            <div className="flex flex-col gap-1">
               <label className="text-[9px] font-bold text-muted-foreground">SUBIR LOGO (Imagen)</label>
        ...
        <Button onClick={handleSave} disabled={loading} className="w-full font-black italic uppercase">
          {loading ? <Loader2 className="animate-spin" /> : "Guardar Agencia Aliada"}
        </Button>
          ...
          {agencias.map(ag => (
            <div key={ag.id} className="p-3 bg-muted/50 rounded-xl border flex justify-between items-center shadow-sm">
              <div className="flex items-center gap-3">
                {ag.logo_url ? (
                  <img src={ag.logo_url} className="w-10 h-10 rounded-lg object-cover border" alt="logo" />
                ) : (
                  <div className="w-10 h-10 bg-muted rounded-lg flex items-center justify-center"><ImageIcon size={16}/></div>
                )}
                <div className="text-left">
                  <p className="font-black text-xs uppercase">{ag.nombre}</p>
                  <p className="text-[9px] opacity-70">📞 {ag.whatsapp}</p>
                </div>
              </div>
