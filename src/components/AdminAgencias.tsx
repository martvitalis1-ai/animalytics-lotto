  ...
  if (!auth) {
    return (
      <div className="p-10 md:p-20 flex flex-col items-center bg-slate-900 rounded-[4rem] border-4 border-emerald-500 shadow-2xl mx-auto max-w-2xl mt-10">
        ...
        <Button onClick={handleAuth} className="bg-emerald-500 w-full h-16 rounded-2xl font-black uppercase text-slate-900 shadow-xl">Entrar al Búnker</Button>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-12 pb-40 animate-in zoom-in duration-500">
      ...
      <div className="bg-slate-900 text-white p-8 rounded-[3rem] border-b-8 border-orange-500 shadow-2xl">
        <h3 className="font-black text-2xl uppercase italic text-orange-500 flex items-center gap-3 mb-8"><ShieldCheck size={30} /> Gestión de Códigos VIP y Acceso</h3>
        <div className="grid md:grid-cols-2 gap-8">
          <div className="space-y-4">
             <p className="font-black text-xs uppercase opacity-50">Código Acceso App</p>
             <div className="flex gap-2"><Input className="bg-slate-800 border-none text-white font-black h-12" /><Button className="bg-emerald-500 text-slate-900 font-black">Añadir</Button></div>
          </div>
          <div className="space-y-4">
             <p className="font-black text-xs uppercase opacity-50">Código VIP Agencia</p>
             <div className="flex gap-2"><Input className="bg-slate-800 border-none text-white font-black h-12" /><Button className="bg-orange-500 text-slate-900 font-black">Generar</Button></div>
          </div>
        </div>
      </div>
