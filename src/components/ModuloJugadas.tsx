...
export function ModuloJugadas() {
  ...
  // ✅ IA QUE NUNCA SE QUEDA PEGADA (CON TRADUCTOR)
            ...
            <div className="grid grid-cols-3 md:grid-cols-6 gap-4 lg:gap-10 items-center justify-items-center">
              {LOTERIAS.map(lot => (
                <button key={lot.id} onClick={() => { setSelectedLot(lot.id); setSelectedHours([]); setSelectedNum(null); }} className={`flex flex-col items-center gap-3 transition-all ${selectedLot === lot.id ? 'scale-110 opacity-100' : 'opacity-40 grayscale-0 hover:opacity-100'}`}>
                  ...
                  <span className={`text-[9px] lg:text-[11px] font-black uppercase text-center ${selectedLot === lot.id ? 'text-emerald-600 underline decoration-8' : 'text-slate-500'}`}>{lot.label}</span>
                </button>
              ))}
            </div>
          </Card>

          <Card className="p-6 lg:p-12 bg-white rounded-[3.5rem] shadow-2xl border-none text-slate-900 text-center text-slate-900">
        ...
        <div className="space-y-8 flex flex-col items-center w-full text-slate-900">
          <div className="lg:sticky lg:top-32 space-y-8 w-full text-slate-900 text-center text-center">
            ...
            <div className="bg-white p-8 lg:p-12 font-mono shadow-2xl rounded-[4rem] border-t-[18px] border-emerald-600 min-h-[500px] flex flex-col text-slate-900 text-center">
              <h4 className="text-center font-black uppercase text-lg border-b border-slate-100 pb-4 mb-8 italic text-center text-slate-900 leading-none">RESUMEN TICKET</h4>
              <div className="flex-1 space-y-5 overflow-y-auto max-h-[350px] no-scrollbar text-center text-slate-900 text-center">
                {currentJugadas.map((j, i) => (
                  <div key={i} className="border-b border-slate-50 pb-5 flex flex-col items-center justify-center text-slate-900">
                    ...
                    <div className="flex items-center gap-4 mt-2 text-slate-900 text-slate-900">
