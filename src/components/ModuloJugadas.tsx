...
// --- LOTERIAS AJUSTADAS A TU BASE DE DATOS ---
const LOTERIAS = [
...
export function ModuloJugadas() {
  ...
  const [selectedLot, setSelectedLot] = useState("lotto_activo"); // ID snake_case
  ...
  // ✅ MOTOR DE IA ESPECÍFICO POR LOTERÍA (CORREGIDO)
  const cargarDatoIA = async () => {
    try {
      if (!selectedLot) return;
      setDatoVip(null); 
      const { data, error } = await supabase.rpc('generar_dato_maestro_vip', {
        lot_name: selectedLot, // Mandamos el ID real: lotto_activo, granjita...
  ...
  const agregar = () => {
    ...
    const labelLot = LOTERIAS.find(l => l.id === selectedLot)?.label || selectedLot;
    setCurrentJugadas([...currentJugadas, { loteria: labelLot, numero: selectedNum, animal: ANIMALS_MASTER[selectedNum], monto: parseFloat(monto), horas: [...selectedHours] }]);
    setSelectedNum(null);
  };
                   ...
                   {isVip ? (
                     ...
                     <div className="flex flex-col items-center gap-8 text-center">
                        ...
                        <div className="w-full max-w-sm flex flex-col gap-4">
                           ...
                           <Button onClick={() => window.open(`https://wa.me/584242542797?text=Quiero comprar un código VIP para el Búnker`, '_blank')} className="w-full h-20 bg-amber-500 text-slate-900 font-black text-xl rounded-3xl uppercase italic shadow-2xl">COMPRAR CÓDIGO VIP</Button>
                        </div>
                     </div>
                   )}
                </div>
