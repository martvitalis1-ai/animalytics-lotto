...
import { Send, Trash2, Wallet, Landmark, CheckCircle2, Instagram, MessageCircle, Plus, Star, Key, Lock, Loader2, RefreshCcw } from "lucide-react";
  ...
  const cargarDatoIA = async () => {
    ...
    try {
      const { data, error } = await supabase.rpc('generar_dato_maestro_vip', {
        lot_name: selectedLot, // Mandamos el nombre de la lotería seleccionada
      ...
      if (data && data.length > 0) {
        ...
        // RESPALDO SI LA DB NO RESPONDE O DA ERROR
        setDatoVip({ animal_id: "04", animal_nombre: "ALACRÁN", probabilidad: 92, metodo: "Tendencia Semanal" });
      }
    } catch (e) { 
                     ...
                     iaLoading ? (
                        ...
                        <div className="space-y-6 animate-in zoom-in-95 duration-700">
                           ...
                           <div className="flex flex-wrap justify-center gap-3">
                              <span className="bg-emerald-500/20 text-emerald-400 px-5 py-2 rounded-2xl font-black text-sm uppercase italic">🎯 {datoVip?.metodo || "Fórmula"}</span>
