...
import { Send, Trash2, Wallet, Landmark, Plus, CheckCircle2, Info, Instagram, MessageCircle, Settings, Save, Image as ImageIcon } from "lucide-react";
import { toast } from "sonner";

// --- LOGOS GITHUB (RUTA EXACTA) ---
...
const LOTERIAS = [
  ...
  { id: "La Granjita", label: "GRANJITA", img: `${IMG_BASE}logo-granjita.png` },
...
export function ModuloJugadas() {
  ...
  const handleSaveAdmin = async () => {
    if (!selectedAgencia) return toast.error("Elige una agencia arriba");
    ...
    if (!error) {
      ...
      toast.error("Error: Asegúrate de haber creado las columnas en Supabase");
    }
  };
  ...
  const agregarJugada = () => {
    ...
    setCurrentJugadas([...currentJugadas, {
      loteria: selectedLot, numero: selectedNum, animal: ANIMALS_GUACHARITO[selectedNum], monto: parseFloat(monto), horas: [...selectedHours]
    }]);
    setSelectedNum(null);
  };
  ...
  const reclamoUrl = useMemo(() => {
    ...
    let tlf = selectedAgencia.whatsapp?.toString().replace(/\D/g, '') || "";
    if (tlf.startsWith('0')) tlf = '58' + tlf.substring(1); else if (!tlf.startsWith('58')) tlf = '58' + tlf;
    const msg = `HOLA, NECESITO REALIZAR UN RECLAMO.\nMIS DATOS: ${userCedula}`;
    return `https://wa.me/${tlf}?text=${encodeURIComponent(msg)}`;
  }, [selectedAgencia, userCedula]);

  if (loading) return <div className="p-20 text-center font-black">Cargando Búnker...</div>;

  return (
    <div className="w-full bg-[#F1F5F9] min-h-screen text-slate-900 pb-40">
      
      {/* --- PANEL SUPERIOR: SELECCION AGENCIA + BOTON ADMIN --- */}
      <div className="w-full bg-[#0F172A] p-6 lg:p-10 text-white shadow-2xl rounded-b-[3rem] mb-10">
        <div className="max-w-7xl mx-auto flex flex-col items-center gap-6">
          <p className="text-[11px] font-black uppercase text-emerald-400 tracking-[0.4em]">PASO 1: SELECCIONA TU AGENCIA</p>
          <div className="flex flex-wrap gap-4 justify-center">
            {agencias.map(ag => (
              <button key={ag.id} onClick={() => { setSelectedAgencia(ag); setAdminIg(ag.instagram_url || ""); setAdminPub(ag.publicidad_url || ""); }} className={`flex items-center gap-3 px-10 py-4 rounded-3xl font-black uppercase text-[12px] transition-all border-2 ${selectedAgencia?.id === ag.id ? 'bg-emerald-600 border-emerald-400 text-white shadow-lg scale-105' : 'bg-slate-800 border-slate-700 text-slate-400'}`}>
          ...
          <Button 
            ...
            className="mt-4 bg-amber-500 hover:bg-amber-600 text-slate-900 font-black uppercase tracking-tighter rounded-2xl h-14 px-8"
          >
            <Settings className="mr-2" size={20}/> CONFIGURAR AGENCIA (SOLO ADMIN)
          </Button>

          {/* FORMULARIO ADMIN DESPLEGABLE */}
          {showAdmin && selectedAgencia && (
            <Card className="w-full max-w-2xl p-8 bg-white rounded-[2rem] border-none shadow-2xl animate-in zoom-in-95 mt-6">
               <h3 className="text-slate-900 font-black text-xl uppercase italic mb-6 flex items-center gap-2">🛠️ Editando: {selectedAgencia.nombre}</h3>
               <div className="space-y-6 text-slate-900">
                  <div className="space-y-2 text-left">
                    <label className="text-[10px] font-black uppercase text-slate-400 ml-2">Link Perfil Instagram</label>
                    <Input value={adminIg} onChange={e => setAdminIg(e.target.value)} placeholder="https://instagram.com/tu_cuenta" className="bg-slate-100 border-none h-14 rounded-2xl font-bold" />
                  </div>
                  <div className="space-y-2 text-left">
                    <label className="text-[10px] font-black uppercase text-slate-400 ml-2">Link Imagen Publicidad (Banner)</label>
                    <Input value={adminPub} onChange={e => setAdminPub(e.target.value)} placeholder="Link de la foto .jpg o .png" className="bg-slate-100 border-none h-14 rounded-2xl font-bold" />
                  </div>
                  <Button onClick={handleSaveAdmin} className="w-full h-16 bg-emerald-600 text-white font-black uppercase rounded-2xl text-lg shadow-xl">
      ...
      <div className="max-w-[1600px] mx-auto grid lg:grid-cols-[1fr_450px] gap-10 px-6">
        
        {/* COLUMNA IZQUIERDA: JUEGO */}
        <div className="space-y-10">
          
          {/* PASO 1: DATOS COBRO (VIP) */}
          <Card className="p-8 lg:p-10 bg-emerald-600 text-white rounded-[3.5rem] shadow-2xl border-none relative overflow-hidden">
             <div className="absolute top-0 right-0 p-10 opacity-10 rotate-12"><Wallet size={150}/></div>
             <div className="relative z-10 space-y-8">
                <div className="text-center lg:text-left">
                   <h2 className="text-2xl font-black uppercase italic tracking-tighter">¿A dónde te pagamos cuando ganes?</h2>
                   <p className="text-[11px] font-bold opacity-80 uppercase tracking-widest mt-1">Llenar estos datos es obligatorio para procesar tus premios.</p>
                </div>
            ...
            <div className="grid grid-cols-3 md:grid-cols-6 gap-6 items-center">
              {LOTERIAS.map(lot => (
                <button key={lot.id} onClick={() => { setSelectedLot(lot.id); setSelectedHours([]); setSelectedNum(null); }} className={`flex flex-col items-center gap-3 transition-all ${selectedLot === lot.id ? 'scale-110 opacity-100' : 'opacity-40 grayscale-0'}`}>
                  ...
                  <span className={`text-[10px] lg:text-[12px] font-black uppercase text-center ${selectedLot === lot.id ? 'text-emerald-600 underline decoration-4' : 'text-slate-500'}`}>{lot.id}</span>
                </button>
              ))}
            </div>
          </Card>
          ...
          <Card className="p-8 lg:p-12 bg-white rounded-[3.5rem] shadow-2xl border-none">
            <div className="grid grid-cols-4 md:grid-cols-6 lg:grid-cols-7 xl:grid-cols-9 gap-3">
              {Object.keys(ANIMALS_GUACHARITO).map(n => (
                <button key={n} onClick={() => setSelectedNum(n)} className={`flex flex-col items-center justify-center p-4 rounded-[2rem] border-2 transition-all h-28 lg:h-36 ${selectedNum === n ? 'border-emerald-500 bg-emerald-50 shadow-inner scale-110 z-10' : 'bg-[#F8FAFC] border-transparent text-slate-600 hover:bg-slate-200'}`}>
                  <span className="text-3xl lg:text-5xl mb-2">{ANIMAL_EMOJIS[n] || '🎟️'}</span>
        ...
        <div className="space-y-8">
          <div className="lg:sticky lg:top-32 space-y-8">
            ...
            {/* CONTACTO AGENCIA */}
            {selectedAgencia && (
              <Card className="p-8 bg-white rounded-[3.5rem] shadow-xl border-2 border-slate-100 flex flex-col gap-4">
                 <p className="text-[10px] font-black uppercase text-slate-400 text-center tracking-[0.2em] italic">CONTACTO Y REDES</p>
                 <div className="grid grid-cols-2 gap-3">
                    <Button onClick={() => selectedAgencia.instagram_url ? window.open(selectedAgencia.instagram_url, '_blank') : toast.error("Instagram no configurado")} className="h-16 rounded-3xl font-black text-[10px] uppercase bg-gradient-to-tr from-amber-400 via-pink-500 to-purple-600 text-white shadow-lg">
            ...
            <div className="bg-white p-8 lg:p-12 font-mono shadow-2xl rounded-[4rem] border-t-[18px] border-emerald-600 min-h-[500px] flex flex-col text-slate-900">
              ...
              <Button 
                onClick={() => {
                   ...
                   if (!selectedAgencia) return toast.error("Selecciona una agencia arriba");
