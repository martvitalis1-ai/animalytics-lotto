...
import { Send, Trash2, Wallet, Landmark, Clock, ReceiptText, Plus, CheckCircle2, ShieldCheck } from "lucide-react";
import { toast } from "sonner";

// --- CONFIGURACIÓN DE LOGOS GITHUB (URL REAL) ---
// Cambia 'Animalytics' y 'animalytics-pro' por tu usuario y repo real
const GITHUB_USER = "Animalytics"; 
const REPO_NAME = "animalytics-pro";
const IMG_BASE = `https://raw.githubusercontent.com/${GITHUB_USER}/${REPO_NAME}/main/public/logos/`;

const LOTERIAS = [
  { id: "Lotto Activo", img: `${IMG_BASE}lotto-activo.png` },
  { id: "La Granjita", img: `${IMG_BASE}la-granjita.png` },
  { id: "Guácharo Activo", img: `${IMG_BASE}guacharo-activo.png` },
  { id: "Guacharito", img: `${IMG_BASE}guacharito.png` },
  { id: "Lotto Rey", img: `${IMG_BASE}lotto-rey.png` },
  { id: "Selva Plus", img: `${IMG_BASE}selva-plus.png` },
];
...
export function ModuloJugadas() {
  ...
  return (
    <div className="max-w-7xl mx-auto p-2 bg-slate-100 min-h-screen text-slate-900 pb-24">
      ...
      {/* SECTOR SUPERIOR: SELECCIÓN DE AGENCIA (MÁXIMA PRIORIDAD) */}
      <div className="p-5 bg-slate-900 text-white shadow-2xl rounded-b-[2rem] mb-4">
        <p className="text-[10px] font-black uppercase text-emerald-400 mb-3 text-center tracking-widest flex items-center justify-center gap-2">
          <ShieldCheck size={14}/> PASO 1: ELIGE TU AGENCIA AUTORIZADA
        </p>
        <div className="flex gap-3 overflow-x-auto pb-2 justify-center">
          {agencias.map(ag => (
            <button 
              ...
              className={`flex items-center gap-2 px-6 py-3 rounded-full font-black uppercase text-[12px] transition-all border-2 ${selectedAgencia?.id === ag.id ? 'bg-emerald-600 border-emerald-400 shadow-[0_0_20px_rgba(16,185,129,0.4)] scale-105' : 'bg-slate-800 border-slate-700 text-slate-500 hover:border-slate-500'}`}
            >
      ...
      {/* SECTOR LOTERÍAS (ICONOS GITHUB) */}
      <div className="p-4 bg-white border-b overflow-x-auto shadow-sm mb-4 rounded-3xl mx-4">
        <div className="flex gap-8 min-w-max px-6 justify-center items-center">
          {LOTERIAS.map(lot => (
            <button 
              key={lot.id} 
            ...
            >
              <div className={`w-20 h-20 rounded-full border-4 ${selectedLot === lot.id ? 'border-emerald-500 shadow-2xl pulse-border' : 'border-slate-100'} overflow-hidden bg-slate-50`}>
                <img src={lot.img} alt={lot.id} className="w-full h-full object-cover" onError={(e: any) => e.target.src = "https://cdn-icons-png.flaticon.com/512/126/126501.png"} />
              </div>
              <span className="text-[11px] font-black uppercase tracking-tighter text-slate-700">{lot.id.split(' ')[0]}</span>
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="grid lg:grid-cols-3 gap-6 p-4">
        ...
        <div className="space-y-6">
          {/* PANEL DE MONTO Y PAGO */}
          <Card className="p-8 bg-white rounded-[3rem] shadow-2xl border-none space-y-6">
            <div className="space-y-1">
              <label className="text-[11px] font-black uppercase opacity-40 ml-1 italic">Monto por Sorteo (Bs)</label>
            ...
            <Button onClick={agregarJugada} className="w-full h-20 bg-emerald-600 hover:bg-emerald-700 text-white font-black uppercase rounded-[2rem] text-xl shadow-[0_15px_30px_-5px_rgba(16,185,129,0.4)] active:scale-95 transition-all">
              <Plus size={28} className="mr-2" /> AÑADIR JUGADA
            </Button>
          </Card>

          {/* TICKET VIRTUAL CON BOTÓN WHATSAPP PREMIUM */}
          <div className="bg-white p-8 font-mono shadow-2xl rounded-3xl border-t-[20px] border-emerald-600 min-h-[500px] flex flex-col text-slate-900 overflow-hidden">
            <div className="text-center border-b-2 border-dashed border-slate-200 pb-4 mb-6">
               <h4 className="font-black uppercase text-xl italic tracking-tighter">Resumen de Ticket</h4>
            </div>
            <div className="flex-1 space-y-4 overflow-y-auto max-h-[350px] pr-2">
              {currentJugadas.map((j, i) => (
                <div key={i} className="border-b border-slate-100 pb-3 flex justify-between items-start animate-in slide-in-from-bottom-2">
                  <div>
                  ...
                  <div className="text-right flex flex-col items-end gap-2">
                    ...
                    <button onClick={() => setCurrentJugadas(prev => prev.filter((_, idx) => idx !== i))} className="text-red-500 bg-red-50 p-2 rounded-xl hover:bg-red-100 transition-colors"><Trash2 size={18}/></button>
                  </div>
                </div>
              ))}
            </div>
            ...
            {/* --- BOTÓN DE WHATSAPP REDISEÑADO (PREMIUM) --- */}
            <a 
              href={msgUrl} 
              ...
              className={`w-full h-24 bg-gradient-to-r from-[#25D366] to-[#1fae53] text-white mt-10 rounded-[2.5rem] font-black text-2xl shadow-[0_20px_40px_-10px_rgba(37,211,102,0.5)] flex items-center justify-center gap-4 transition-all hover:scale-[1.03] active:scale-95 relative overflow-hidden group ${msgUrl === "#" ? 'opacity-20 pointer-events-none grayscale' : ''}`}
            >
      ...
      {/* FOOTER DE DATOS PERSISTENTES */}
      <div className="fixed bottom-0 left-0 right-0 bg-slate-900 p-5 border-t-4 border-emerald-500 flex gap-3 z-50 shadow-[0_-10px_30px_rgba(0,0,0,0.3)]">
          <Input value={userBanco} onChange={e => setUserBanco(e.target.value)} placeholder="Tu Banco" className="bg-slate-800 border-none text-white font-black h-14 text-sm rounded-2xl" />
      ...
      <style>{`
        .pulse-border { animation: pulse-emerald 2s infinite; }
