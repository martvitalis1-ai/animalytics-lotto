...
import { Send, Trash2, Wallet, CheckCircle2, Instagram, MessageCircle, Plus, Star, Lock, Loader2 } from "lucide-react";
import { toast } from "sonner";

// --- RUTA GITHUB ---
const IMG_BASE = "https://raw.githubusercontent.com/martvitalis1-ai/animalytics-lotto/main/src/assets/";

// IDs exactamente como están en tu base de datos (snake_case)
const LOTERIAS = [
...
export function ModuloJugadas() {
  ...
  const [selectedLot, setSelectedLot] = useState("lotto_activo"); // ID inicial corregido
  ...
  const cargarDatoIA = async () => {
    ...
    try {
      ...
      else setDatoVip({ animal_id: "07", animal_nombre: "PERICO", probabilidad: 95, metodo: "Tendencia" });
    } catch (e) { 
      setDatoVip({ animal_id: "12", animal_nombre: "CABALLO", probabilidad: 90, metodo: "Fórmula" });
  ...
  const agregar = () => {
    ...
    setCurrentJugadas([...currentJugadas, { loteria: label, numero: selectedNum, animal: ANIMALS_MASTER[selectedNum] || "Animal", monto: parseFloat(monto), horas: [...selectedHours] }]);
    setSelectedNum(null);
  };
  ...
  const msgUrl = useMemo(() => {
    ...
    currentJugadas.forEach(j => { msg += `${j.loteria}\nAnimal: ${j.numero} - ${j.animal}\nSorteos: ${j.horas.join(", ")}\nMonto: ${j.monto} Bs x sorte\n----------\n`; });
  ...
  return (
    <div className="w-full bg-[#F8FAFC] min-h-screen text-slate-900 pb-40 overflow-x-hidden flex flex-col items-center">
      ...
      <div className="w-full bg-[#0F172A] p-6 lg:p-10 text-white shadow-2xl rounded-b-[3rem] mb-10 flex flex-col items-center">
        <p className="text-[10px] font-black uppercase text-emerald-400 mb-6 tracking-[0.4em] text-center italic">PASO 1: SELECCIONA TU AGENCIA</p>
          ...
          <Card className="p-8 lg:p-12 bg-slate-900 border-none shadow-2xl rounded-[4rem] overflow-hidden relative border-t-[12px] border-emerald-500">
             ...
             <div className="relative z-10 space-y-8 text-white">
                <div className="flex justify-between items-center px-4">
                  <span className="bg-emerald-500 text-slate-900 px-6 py-2 rounded-full font-black text-[10px] uppercase tracking-widest animate-pulse">PRÓXIMO DATO VIP</span>
                  <span className="text-emerald-400 font-black text-2xl italic tracking-tighter uppercase underline">{LOTERIAS.find(l => l.id === selectedLot)?.label || "LOTERIA"}</span>
                </div>
                   ...
                   {isVip ? (
                     iaLoading ? (
                        <div className="flex flex-col items-center gap-4 py-10">
                           ...
                           <p className="font-black uppercase tracking-widest text-emerald-500 text-sm">Analizando 4 sorteos...</p>
                        </div>
                     ) : (
                        <div className="space-y-6 animate-in zoom-in-95 duration-700">
                           ...
                           <div className="flex flex-wrap justify-center gap-3">
                              <span className="bg-emerald-500/20 text-emerald-400 px-5 py-2 rounded-2xl font-black text-xs uppercase italic">🎯 {datoVip?.metodo || "Fórmula"}</span>
                              <span className="bg-white/10 text-white px-5 py-2 rounded-2xl font-black text-xs uppercase italic">🔥 {datoVip?.probabilidad || "95"}% ÉXITO</span>
                           </div>
                        </div>
                     )
                   ) : (
        ...
        <div className="space-y-8 flex flex-col items-center w-full text-slate-900">
          <div className="lg:sticky lg:top-32 space-y-8 w-full text-center">
            ...
            <div className="bg-white p-8 lg:p-12 font-mono shadow-2xl rounded-[4rem] border-t-[18px] border-emerald-600 min-h-[500px] flex flex-col text-slate-900">
              <h4 className="text-center font-black uppercase text-lg border-b border-slate-100 pb-4 mb-8 italic text-center leading-none">RESUMEN TICKET</h4>
