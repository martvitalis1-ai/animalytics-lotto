...
import { Send, Plus, Trash2, Pencil, Landmark, Wallet, CheckCircle2, History, Clock, ReceiptText } from "lucide-react";
...
export function ModuloJugadas() {
  ...
  const mandarWhatsApp = (t: any) => {
    ...
    const url = `https://wa.me/${t.agenciaWhatsapp}?text=${encodeURIComponent(ticket)}`;
    window.open(url, '_blank');
  };

  const finalizarTicket = () => {
    if (!selectedAgencia || currentJugadas.length === 0 || !userPM) return toast.error("Faltan datos bancarios o agencia");
  ...
  if (loading) return <div className="p-20 text-center animate-pulse uppercase font-black italic">Conectando...</div>;
      ...
      <section className="grid lg:grid-cols-2 gap-8">
        <div className="space-y-6">
          ...
          <div className="space-y-2">
            <label className="text-[10px] font-black uppercase text-muted-foreground">Paso 1: Elige Agencia</label>
          ...
          {selectedAgencia && (<div className="p-4 bg-emerald-500/5 border-2 border-emerald-500/20 rounded-2xl text-[11px] font-black uppercase italic space-y-1"><p className="text-emerald-600 text-[8px]">🏦 Pagar a Agencia:</p><p>{selectedAgencia.banco_nombre} | {selectedAgencia.banco_cedula} | {selectedAgencia.banco_telefono}</p></div>)}
          <Card className="border-2 rounded-3xl border-primary/20 p-5 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-2"><Input placeholder="Nº Animal" value={num} onChange={e => setNum(e.target.value)} className="h-11 font-black text-center text-xl" /><Input placeholder="Monto Bs" type="number" value={mon} onChange={e => setMon(e.target.value)} className="h-11 font-black text-center text-xl" /></div>
