      ...
      <div className="bg-white border-4 border-slate-900 rounded-[2.5rem] p-6 shadow-xl">
         <div className="flex justify-center flex-wrap gap-4">
            {['la_granjita', 'lotto_activo', 'el_guacharo', 'guacharito', 'selva_plus', 'lotto_rey'].map(lot => (
              <button key={lot} onClick={() => setSelectedLottery(lot)} className={`p-1.5 rounded-full ring-4 transition-all ${selectedLottery === lot ? 'ring-emerald-500 bg-slate-900 shadow-xl' : 'ring-transparent bg-black opacity-50'}`}>
