  ...
  const cargarDatoIA = async () => {
    try {
      ...
      if (data && data.length > 0) {
        ...
        setDatoVip({ animal_id: "04", animal_nombre: "ALACRÁN", probabilidad: 94, metodo: "Tendencia" });
      }
    } catch (e) { 
                   ...
                   {isVip ? (
                     ...
                     <div className="flex flex-col items-center gap-8 text-center">
                        ...
                        <div className="w-full max-w-sm flex flex-col gap-4">
                           ...
                           <Button onClick={() => window.open(`https://wa.me/584242542797?text=Quiero activar mi Búnker VIP`, '_blank')} className="w-full h-20 bg-amber-500 text-slate-900 font-black text-xl rounded-3xl uppercase italic shadow-2xl hover:scale-105 transition-all">COMPRAR CÓDIGO VIP</Button>
                        </div>
                     </div>
                   )}
                </div>
