...
export function ModuloJugadas() {
  ...
  // --- FUNCIÓN DE SUPER-CODIFICACIÓN (EL SECRETO) ---
  const superEncode = (str: string) => {
    return encodeURIComponent(str)
      .replace(/\*/g, '%2A') // Codifica el asterisco (esto es lo que lo cortaba)
      .replace(/'/g, '%27')  // Codifica comilla simple
      .replace(/\(/g, '%28') // Codifica paréntesis
      .replace(/\)/g, '%29') // Codifica paréntesis
      .replace(/!/g, '%21')  // Codifica exclamación
      .replace(/~/g, '%7E'); // Codifica tilde
  };

  const handleSendWhatsApp = () => {
    if (!selectedAgencia || currentJugadas.length === 0 || !userPM) {
        return toast.error("Completa los datos primero");
    }

    // 1. Número limpio sin símbolos
    ...
    // 2. Construcción del mensaje con formato de agencia
    ...
    // 4. ENVÍO BLINDADO
    // Usamos wa.me que es el protocolo más ligero para el handoff del móvil
    const finalUrl = `https://wa.me/${tlf}?text=${superEncode(msg)}`;
    
    // Forzamos la apertura directa
      ...
      <section className="grid lg:grid-cols-2 gap-8">
        <div className="space-y-6">
          <div className="bg-primary/5 p-5 rounded-3xl border border-primary/10 space-y-4">
            <label className="text-[10px] font-black uppercase text-primary">Tus Datos de Cobro</label>
          ...
          <div className="space-y-2">
            <label className="text-[10px] font-black uppercase text-muted-foreground">Agencia</label>
            <div className="flex flex-wrap gap-2">
              {agencias.map(ag => (
                <button key={ag.id} onClick={() => setSelectedAgencia(ag)} className={`px-3 py-2 border-2 rounded-xl text-[10px] font-black uppercase ${selectedAgencia?.id === ag.id ? 'border-primary bg-primary/10 shadow-lg' : 'bg-card border-border'}`}>
                  {ag.nombre}
                </button>
              ))}
            </div>
          ...
          <Card className="border-2 rounded-3xl p-5 space-y-4 border-primary/20">
        ...
        <div className="space-y-4">
          <div className="bg-white text-slate-900 p-8 font-mono shadow-2xl rounded-sm border-t-[14px] border-primary min-h-[450px]">
            ...
            <div className="mt-8 flex justify-between font-black text-xl border-t-2 pt-4">
              <span>TOTAL:</span>
              <span className="text-emerald-700 underline decoration-double">{currentJugadas.reduce((acc, curr) => acc + (curr.monto * curr.horas.length), 0).toFixed(2)} Bs</span>
            </div>
