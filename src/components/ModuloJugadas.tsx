...
import { Send, Plus, Trash2, Wallet, Clock, ReceiptText } from "lucide-react";
...
export function ModuloJugadas() {
  ...
  const [userBanco, setUserBanco] = useState(() => localStorage.getItem('u_p_banco') || "");
  const [userPM, setUserPM] = useState(() => localStorage.getItem('u_p_tlf') || "");
  const [userCedula, setUserCedula] = useState(() => localStorage.getItem('u_p_cedula') || "");
  ...
  const whatsappUrl = useMemo(() => {
    ...
    let msg = `*JUGADA - ANIMALYTICS PRO*\n----------------------------\n*DATOS DE COBRO:*\n🏦 Banco: ${userBanco}\n📞 PM: ${userPM}\n🆔 CI: ${userCedula}\n----------------------------\n`;
      ...
      <section className="grid lg:grid-cols-2 gap-8">
        <div className="space-y-6">
          <div className="bg-primary/5 p-5 rounded-3xl border border-primary/10 space-y-4 shadow-inner">
            ...
            <Input value={userBanco} onChange={e => { setUserBanco(e.target.value); localStorage.setItem('u_p_banco', e.target.value); }} placeholder="Tu Banco" className="bg-background font-bold text-xs" />
            <div className="grid grid-cols-2 gap-2">
                <Input value={userPM} onChange={e => { setUserPM(e.target.value); localStorage.setItem('u_p_tlf', e.target.value); }} placeholder="Tlf Pago Móvil" className="bg-background font-bold text-xs" />
                <Input value={userCedula} onChange={e => { setUserCedula(e.target.value); localStorage.setItem('u_p_cedula', e.target.value); }} placeholder="Tu Cédula" className="bg-background font-bold text-xs" />
            </div>
          </div>
          <div className="space-y-2">
            <label className="text-[10px] font-black uppercase text-muted-foreground flex items-center gap-2">1. Selecciona Agencia</label>
            <div className="flex flex-wrap gap-2">{agencias.map(ag => (<button key={ag.id} onClick={() => setSelectedAgencia(ag)} className={`px-4 py-2 border-2 rounded-xl text-[10px] font-black uppercase transition-all flex items-center gap-2 ${selectedAgencia?.id === ag.id ? 'border-primary bg-primary/10 shadow-lg' : 'bg-card border-border'}`}>{ag.logo_url && <img src={ag.logo_url} className="w-6 h-6 rounded-full object-cover border" alt="logo" />}{ag.nombre}</button>))}</div>
          </div>
          {selectedAgencia && (<div className="p-4 bg-emerald-500/5 border-2 border-emerald-500/20 rounded-2xl text-[11px] font-black uppercase italic space-y-1 shadow-sm"><p className="text-emerald-600 text-[8px] uppercase tracking-widest">🏦 Pagar a Agencia:</p><p>{selectedAgencia.banco_nombre || '---'} | {selectedAgencia.banco_cedula} | {selectedAgencia.banco_telefono}</p></div>)}
          <Card className="border-2 rounded-3xl border-primary/20 p-5 space-y-4 shadow-xl">
             ...
             <div className="grid grid-cols-2 gap-2"><Input placeholder="Animal Nº" value={num} onChange={e => setNum(e.target.value)} className="h-11 font-black text-center text-xl border-primary/10" /><Input placeholder="Bs" type="number" value={mon} onChange={e => setMon(e.target.value)} className="h-11 font-black text-center text-xl border-primary/10" /></div>
        ...
        <div className="space-y-4">
          <div className="bg-white text-slate-900 p-8 font-mono shadow-2xl rounded-sm border-t-[14px] border-primary min-h-[450px] relative text-left">
             <div className="text-center border-b-2 border-dashed pb-4 mb-6">{selectedAgencia?.logo_url && <img src={selectedAgencia.logo_url} className="w-14 h-14 mx-auto mb-2 rounded-full border shadow-sm object-cover bg-muted" alt="logo" />}<h3 className="font-black text-2xl uppercase tracking-tighter">{selectedAgencia?.nombre || "NUEVA JUGADA"}</h3><p className="text-[9px] opacity-60 uppercase font-black tracking-widest font-bold">Animalytics Pro Ticket</p></div>
             <div className="space-y-5">
               {currentJugadas.map((j, i) => (<div key={i} className={`text-[12px] border-b border-dotted pb-3 flex justify-between items-center text-left`}><div className="flex-1">
                 ...
                 <p className="text-[9px] opacity-70 italic leading-none">Sorteos: {j.horas.join(", ")}</p></div><button onClick={() => setCurrentJugadas(currentJugadas.filter((_, idx) => idx !== i))} className="text-red-600 p-1 ml-4"><Trash2 size={18}/></button></div>))}
             ...
             <a href={whatsappUrl} target="_blank" rel="noopener noreferrer" className={`w-full h-16 bg-[#25D366] hover:bg-[#20ba5a] text-white mt-10 rounded-2xl font-black text-lg shadow-2xl flex items-center justify-center gap-3 transition-all hover:scale-[1.02] active:scale-95 ${(!selectedAgencia || currentJugadas.length === 0 || !userPM) ? 'pointer-events-none opacity-50' : ''}`}><Send size={24} /> ENVIAR TICKET POR WHATSAPP</a>
          </div>
        </div>
      </section>
    </div>
