...
export function ModuloJugadas() {
  ...
  useEffect(() => {
    const init = async () => {
      try {
        ...
        const local = localStorage.getItem('tickets_history_final_fix');
  ...
  const mandarWhatsApp = (t: any) => {
    ...
    msg += `*TOTAL A PAGAR:* ${t.total.toFixed(2)} Bs\n`;
    msg += `----------------------------\n`;
    msg += `📥 _Enviado desde App Animalytics._`;

    // SOLUCIÓN AL BLOQUEO DE OPERA:
    // 1. Limpiar el número de teléfono (solo números)
    const phoneClean = t.agenciaWhatsapp.toString().replace(/\D/g, '');
    
    // 2. Usar el link wa.me que es universal y no lo bloquean
    const finalUrl = `https://wa.me/${phoneClean}?text=${encodeURIComponent(msg)}`;

    // 3. Crear un enlace invisible y hacerle click (esto engaña al bloqueador de Opera)
  ...
  const agregarOActualizar = () => {
    if (!lot || !num || !mon || selectedHours.length === 0) {
      return toast.error("Completa los datos de la jugada");
    }
    const nueva: Play = {
      loteria: lot,
  ...
  const finalizarTicket = () => {
    if (!selectedAgencia || currentJugadas.length === 0 || !userPM || !userBanco) {
      return toast.error("Faltan datos bancarios o agencia");
    }
    ...
    localStorage.setItem('tickets_history_final_fix', JSON.stringify(updatedHistory));
