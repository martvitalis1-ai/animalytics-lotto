          ...
          <div className="flex gap-4 min-w-max px-4 justify-start">
            {LOTERIAS.map(lot => (
              <button 
                ...
                className={`flex flex-col items-center gap-2 transition-all ${selectedLot === lot.id ? 'scale-105' : 'opacity-60'}`}
              >
            ...
            {selectedAgencia && (
              <div className="mt-4 p-4 bg-emerald-50 border-2 border-emerald-500/20 rounded-2xl text-left">
                ...
                <p className="text-[10px] font-bold text-slate-600">
                  CI: {selectedAgencia.banco_cedula || selectedAgencia.cedula_banco || "---"}
                </p>
              </div>
            )}
