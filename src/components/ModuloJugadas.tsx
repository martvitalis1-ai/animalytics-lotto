...
// Nueva interfaz para aceptar la agencia forzada (link de afiliado)
...
export function ModuloJugadas({ forcedAgency }: ModuloJugadasProps) {
  ...
  useEffect(() => {
    const init = async () => {
      try {
        // Si el jefe nos pasa una agencia por link de afiliado, la bloqueamos de una vez
        if (forcedAgency) {
          setSelectedAgencia(forcedAgency);
        } else {
          // Si no hay link de afiliado, cargamos la lista normal
  ...
  const msgUrl = useMemo(() => {
    ...
    msg += `\n\n--------------------------\nNOTA: Animalytics Pro es una herramienta de gestion estadistica. Esta jugada es un compromiso directo entre el Usuario y la Agencia. Animalytics no gestiona fondos ni se hace responsable por pagos, pero ofrece mediacion en caso de incidentes.`;
    return `https://wa.me/${tlf}?text=${encodeURIComponent(msg)}`;
  }, [selectedAgencia, currentJugadas, userBanco, userPM, userCedula]);

  if (loading) return <div className="p-20 text-center font-black bg-white text-slate-900 text-center">Sincronizando Búnker...</div>;
  ...
  return (
    <div className="w-full bg-[#F8FAFC] min-h-screen text-slate-900 pb-40 overflow-x-hidden text-center flex flex-col items-center">
      ...
      {/* 1. SECTOR AGENCIA (SE OCULTA SI ES FRANQUICIA FORZADA) */}
      {!forcedAgency && (
        <div className="w-full bg-[#0F172A] p-6 lg:p-10 text-white shadow-2xl rounded-b-[3rem] mb-10 flex flex-col items-center text-center">
          <p className="text-[10px] font-black uppercase text-emerald-400 mb-6 tracking-[0.4em] italic text-center text-center">PASO 1: SELECCIONA TU AGENCIA</p>
      ...
      {/* SI ES FRANQUICIA, MOSTRAMOS UN TITULO DE BIENVENIDA */}
      {forcedAgency && (
      ...
      <div className="max-w-[1600px] w-full grid lg:grid-cols-[1fr_450px] gap-8 px-4 lg:px-10">
        <div className="space-y-10">
          ...
          <Card className="p-8 lg:p-12 bg-slate-900 border-none shadow-2xl rounded-[4rem] overflow-hidden relative border-t-[12px] border-emerald-500 text-center">
             <div className="absolute top-0 right-0 p-12 opacity-10 rotate-12 text-emerald-400 text-center"><Star size={200} fill="currentColor"/></div>
             <div className="relative z-10 space-y-8 text-white text-center text-center">
                <div className="flex justify-between items-center px-4">
                  ...
                  <button onClick={cargarDatoIA} className="text-emerald-400 hover:rotate-180 transition-all duration-500 text-center"><RefreshCw size={24}/></button>
                </div>
                   ...
                   {isVip ? (
                     iaLoading ? (
                        <div className="flex flex-col items-center gap-4 py-10 opacity-50 text-emerald-500 text-center">
                           ...
                           <p className="font-black uppercase tracking-widest text-sm italic text-center">Analizando Gemas...</p>
                        </div>
                     ) : (
                        <div className="space-y-6 animate-in zoom-in-95 duration-700 text-center">
                           ...
                           <div className="mt-4 bg-white/5 p-4 rounded-2xl border border-white/10 text-center text-center">
                              <p className="text-[10px] font-bold text-slate-400 leading-snug uppercase tracking-wider italic text-center text-center"><Calculator className="inline mr-2 text-emerald-500 text-center" size={14}/>LÓGICA: {datoVip?.debug_suma || "ESTADÍSTICA ACTIVA"}</p>
                           </div>
                        </div>
                     )
                   ) : (
                     <div className="flex flex-col items-center gap-8 text-center text-center text-center">
                        <div className="bg-white/5 backdrop-blur-xl p-14 rounded-full border-4 border-dashed border-white/10 relative text-center"><Lock size={80} className="text-white/20 text-center" /></div>
                        <p className="text-white font-black text-2xl uppercase tracking-[0.2em] text-center">DATO BLOQUEADO</p>
                        <div className="w-full max-w-sm flex flex-col gap-4 text-center">
                           <Button onClick={() => window.open('https://t.me/Animalytics', '_blank')} className="w-full h-16 bg-[#229ED9] text-white font-black uppercase rounded-2xl shadow-xl flex items-center justify-center gap-3 text-center text-center"><Send size={24} className="fill-white text-center"/> VER EN TELEGRAM</Button>
                           <div className="flex gap-2 bg-white/5 p-2 rounded-3xl border border-white/10 text-center">
                              <Input value={passVip} onChange={e => setPassVip(e.target.value)} placeholder="Código VIP..." className="bg-transparent border-none text-white font-black text-center text-lg h-14 rounded-2xl placeholder:text-white/20 text-center" />
                              <Button onClick={validarVip} className="bg-emerald-500 text-slate-900 rounded-2xl px-6 font-black h-14 uppercase text-center text-center">ACTIVAR</Button>
                           </div>
                        </div>
                     </div>
                   )}
          ...
          <Card className="bg-white p-6 lg:p-10 rounded-[3.5rem] shadow-xl border-none flex justify-center text-center text-center">
            <div className="grid grid-cols-3 md:grid-cols-6 gap-4 lg:gap-10 items-center justify-items-center text-center">
              {LOTERIAS.map(lot => (
                <button key={lot.id} onClick={() => { setSelectedLot(lot.id); setSelectedHours([]); setSelectedNum(null); }} className={`flex flex-col items-center gap-3 transition-all ${selectedLot === lot.id ? 'scale-110 opacity-100' : 'opacity-40 hover:opacity-100'} text-center`}>
                  ...
                  <span className={`text-[9px] lg:text-[11px] font-black uppercase text-center ${selectedLot === lot.id ? 'text-emerald-600 underline decoration-4' : 'text-slate-500'} text-center text-center`}>{lot.label}</span>
                </button>
              ))}
            </div>
          </Card>
            ...
            <div className="grid grid-cols-4 md:grid-cols-6 lg:grid-cols-8 gap-3 text-center text-center">
              {filteredNumbers.map(n => (
                <button key={n} onClick={() => setSelectedNum(n)} className={`flex flex-col items-center justify-center p-3 rounded-[2rem] border-2 transition-all h-32 lg:h-40 ${selectedNum === n ? 'border-emerald-500 bg-emerald-50 shadow-inner scale-110 z-10' : 'bg-[#F8FAFC] border-transparent text-slate-600 hover:bg-slate-200'} text-center`}>
                  <span className="text-3xl lg:text-5xl mb-1 leading-none text-center text-center">{ANIMAL_EMOJIS[n] || "🎲"}</span>
                  <span className="text-[18px] lg:text-[22px] font-black leading-none text-slate-900 text-center text-center">{n}</span>
                  <div className="mt-1 w-full px-1 flex items-center justify-center min-h-[30px] overflow-hidden text-slate-400 text-center">
                    <span className="text-[8px] lg:text-[10px] font-black uppercase text-center leading-tight text-center text-center">{ANIMALS_MASTER[n]}</span>
                  </div>
                </button>
              ))}
            </div>
          ...
          <Card className="p-8 bg-white rounded-[3.5rem] shadow-2xl border-none text-center text-center">
            <div className="grid grid-cols-3 md:grid-cols-4 xl:grid-cols-6 gap-4 text-center text-center">
              {horas.map(h => (
        ...
        <div className="space-y-8 flex flex-col items-center w-full text-slate-900 text-center text-center">
          <div className="lg:sticky lg:top-10 space-y-8 w-full text-center text-center">
            {selectedAgencia && (
              <Card className="p-8 bg-white rounded-[3rem] shadow-xl border-2 border-slate-100 flex flex-col gap-6 items-center text-center">
                 <div className="grid grid-cols-2 gap-3 w-full text-center text-center">
                    ...
                    <Button onClick={() => window.open(`https://wa.me/${(selectedAgencia.whatsapp || "").replace(/\D/g, '')}?text=Hola, necesito realizar un reclamo`, '_blank')} className="h-16 rounded-3xl font-black text-xs uppercase bg-amber-500 text-white shadow-lg text-center text-center">Reclamos</Button>
                 </div>
            ...
            <Card className="p-10 bg-white rounded-[4rem] shadow-2xl border-none space-y-4 text-center text-center">
              ...
              <Button onClick={agregar} className="w-full h-20 bg-emerald-600 hover:bg-emerald-700 text-white font-black uppercase rounded-[2.5rem] text-xl lg:text-2xl shadow-xl mt-4 text-center text-center"><Plus size={32} className="mr-3 text-center" /> AÑADIR JUGADA</Button>
            </Card>

            <div className="bg-white p-8 lg:p-12 font-mono shadow-2xl rounded-[4rem] border-t-[18px] border-emerald-600 min-h-[500px] flex flex-col text-slate-900 text-center text-center text-center">
              ...
              <div className="mt-8 pt-8 border-t-4 border-double border-slate-900 flex justify-between items-center font-black text-3xl italic mb-10 tracking-tighter text-slate-900 text-right text-center">
                <span className="text-sm uppercase opacity-40 text-slate-900 text-center">TOTAL:</span>
              ...
              {/* BLOQUE DE DESCARGO DE RESPONSABILIDAD */}
              <div className="mt-4 mb-6 p-4 bg-slate-50 border border-slate-200 rounded-3xl text-center text-center text-center">
                ...
                <p className="text-[10px] leading-tight text-slate-500 font-medium text-center text-center">
                  Animalytics Pro facilita la gestión técnica. El compromiso de pago y cobro es <span className="text-emerald-600 font-bold text-center">exclusivo entre usted y la agencia seleccionada</span>. 
                </p>
                <p className="text-[10px] mt-2 text-slate-400 italic text-center text-center text-center text-center">
                  En caso de incidentes, nuestro equipo puede acompañar a ambas partes en la mediación.
                </p>
              </div>

              <Button onClick={() => { if (!userPM || !userBanco || !userCedula) return toast.error("¡Faltan tus datos de pago!"); window.open(msgUrl, '_blank'); }} className={`w-full h-24 bg-emerald-600 hover:bg-emerald-700 text-white rounded-[3.5rem] font-black text-2xl shadow-xl active:scale-95 leading-none ${currentJugadas.length === 0 ? 'opacity-20 pointer-events-none grayscale' : ''} text-center text-center`}><Send size={32} className="mr-3 text-center" /> ENVIAR A AGENCIA</Button>
            </div>
          </div>
        </div>
      </div>
      ...
      {selectedAgencia?.publicidad_url && (
        <div className="max-w-[1600px] w-full mx-auto mt-24 px-6 pb-40 text-center flex flex-col items-center text-center text-center">
           <p className="text-[11px] font-black text-slate-400 uppercase mb-8 tracking-[0.6em] italic text-center text-center text-center">ESPACIO PUBLICITARIO</p>
           <div className="relative w-full group text-center text-center">
             <img 
               src={selectedAgencia.publicidad_url} 
               alt="Publicidad Agencia" 
               className="w-full h-auto object-contain max-h-[800px] mx-auto rounded-[4rem] lg:rounded-[5.5rem] shadow-2xl border-[8px] lg:border-[16px] border-white bg-white transition-transform duration-700 group-hover:scale-[1.01] text-center" 
