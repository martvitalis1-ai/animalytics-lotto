...
import { Send, Trash2, Wallet, Landmark, CheckCircle2, Instagram, MessageCircle, Plus, Star, Key, Lock, AlertCircle, Loader2 } from "lucide-react";
...
export function ModuloJugadas() {
  ...
  const cargarDatoIA = async () => {
    try {
      ...
      setDatoVip({ animal_id: "07", animal_nombre: "PERICO", probabilidad: 85, metodo: "Tendencia" });
    }
  };
  ...
  const validarVip = async () => {
    if (!passVip) return toast.error("Ingresa tu código");
    setValidando(true);
    try {
    ...
    } catch (err) { toast.error("Error al validar"); }
  ...
  const agregar = () => {
    ...
    setCurrentJugadas([...currentJugadas, { 
      loteria: selectedLot, 
      numero: selectedNum, 
      animal: ANIMALS_MASTER[selectedNum] || "Animal", 
                   ...
                   {isVip ? (
                     ...
                     <div className="flex flex-col items-center gap-8 text-center">
                        ...
                        <div className="w-full max-w-sm flex flex-col gap-4">
                           ...
                           <Button onClick={() => window.open(`https://wa.me/584242542797?text=Hola, quiero activar mi Búnker VIP`, '_blank')} className="w-full h-20 bg-amber-500 text-slate-900 font-black text-xl rounded-3xl uppercase italic shadow-2xl hover:scale-105 transition-all">SOLICITAR CÓDIGO VIP</Button>
                        </div>
                     </div>
                   )}
                </div>
        ...
        <div className="space-y-8 flex flex-col items-center w-full">
          <div className="lg:sticky lg:top-32 space-y-8 w-full text-slate-900 text-center">
            ...
            <div className="bg-white p-8 lg:p-12 font-mono shadow-2xl rounded-[4rem] border-t-[18px] border-emerald-600 min-h-[500px] flex flex-col">
              ...
              <div className="mt-8 pt-8 border-t-4 border-double border-slate-900 flex justify-between items-center font-black text-3xl italic mb-10 text-right text-slate-900">
