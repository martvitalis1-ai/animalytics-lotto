...
// --- IDS SINCRONIZADOS CON TU BASE DE DATOS ---
const LOTERIAS = [
  ...
  { id: "guacharo", label: "GUÁCHARO", img: `${IMG_BASE}logo-guacharito.png` }, // Dados
...
export function ModuloJugadas() {
  ...
  const [selectedLot, setSelectedLot] = useState("lotto_activo"); // ID exacto
  ...
  const cargarDatoIA = async () => {
    ...
    try {
      const { data, error } = await supabase.rpc('generar_dato_maestro_vip', {
        lot_id: selectedLot, // Mandamos el ID exacto (lotto_activo, granjita...)
        proxima_hora: horaQuery
      });
      if (data && data.length > 0) setDatoVip(data[0]);
    } catch (e) { 
        console.warn("IA esperando resultados...");
        ...
        <div className="space-y-10 text-center">
          ...
          <Card className="p-8 lg:p-12 bg-emerald-600 text-white rounded-[3.5rem] shadow-2xl border-none relative overflow-hidden flex flex-col items-center text-center">
             ...
             <div className="relative z-10 w-full max-w-2xl space-y-8">
                <h2 className="text-2xl lg:text-3xl font-black uppercase italic tracking-tighter text-center leading-tight text-white text-center">¿DÓNDE TE ENVIAMOS TU PAGO?</h2>
                   ...
                   {isVip ? (
                     iaLoading ? (
                        <div className="flex flex-col items-center gap-4 py-10 opacity-50 text-emerald-500">
                           ...
                           <p className="font-black uppercase tracking-widest text-emerald-500 text-sm italic text-center">Analizando 4 sorteos...</p>
                        </div>
                     ) : (
                        <div className="space-y-6 animate-in zoom-in-95 duration-700">
                           ...
                           <div className="flex flex-wrap justify-center gap-3">
                              <span className="bg-emerald-500/20 text-emerald-400 px-5 py-2 rounded-2xl font-black text-sm uppercase italic tracking-widest flex items-center gap-2"><Target size={16}/> {datoVip?.metodo}</span>
