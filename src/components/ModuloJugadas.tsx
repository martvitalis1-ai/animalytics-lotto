...
// --- RUTA EXACTA DE TUS LOGOS EN GITHUB ---
...
export function ModuloJugadas() {
  ...
  return (
    <div className="max-w-7xl mx-auto bg-[#F8FAFC] min-h-screen text-slate-900 pb-40">
      ...
      {/* 2. SELECTOR DE LOTERÍA (FIX: LOTTO ACTIVO) */}
          ...
          <div className="flex gap-6 min-w-max px-4">
            {LOTERIAS.map(lot => (
              ...
              >
                ...
                <span className="text-[9px] font-black uppercase text-slate-600 tracking-tighter">{lot.label}</span>
              </button>
            ))}
          </div>
        </div>
      ...
      <div className="grid lg:grid-cols-3 gap-4 p-4">
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white p-6 font-mono shadow-2xl rounded-[2.5rem] border-t-[14px] border-emerald-600 flex flex-col text-slate-900">
            ...
            {selectedAgencia && (
              <div className="mt-4 p-4 bg-emerald-50 border-2 border-emerald-500/20 rounded-2xl animate-in zoom-in-95">
                <p className="text-[9px] font-black text-emerald-600 uppercase flex items-center gap-1"><Info size={10}/> DATOS DE PAGO AGENCIA:</p>
                <p className="text-[11px] font-black text-slate-800 uppercase italic mt-1 leading-none">
                  {selectedAgencia.banco_nombre || selectedAgencia.nombre_banco || "Banco no definido"}
                </p>
                <p className="text-[10px] font-bold text-slate-600 mt-1">
                  Tlf: {selectedAgencia.banco_telefono || selectedAgencia.telefono_banco || "---"}
                </p>
                <p className="text-[10px] font-bold text-slate-600">
                  ID: {selectedAgencia.banco_cedula || selectedAgencia.cedula_banco || "---"}
                </p>
              </div>
            )}
