...
export function ModuloJugadas() {
  ...
  const cargarDatoIA = async () => {
    ...
    } catch (e) { console.warn("IA esperando..."); }
  ...
  const validarVip = async () => {
    ...
    try {
      ...
      if (data) {
        ...
        toast.success("¡BÚNKER VIP ACTIVADO!");
  ...
  return (
    <div className="w-full bg-[#F8FAFC] min-h-screen text-slate-900 pb-40 overflow-x-hidden">
      ...
      {/* 1. SECTOR AGENCIA - FULL WIDTH */}
      <div className="w-full bg-[#0F172A] p-6 lg:p-10 text-white shadow-2xl lg:rounded-b-[4rem] mb-10 text-center flex flex-col items-center">
        ...
        <div className="flex flex-wrap gap-4 justify-center">
          {agencias.map(ag => (
            <button key={ag.id} onClick={() => setSelectedAgencia(ag)} className={`flex items-center gap-2 px-10 py-4 rounded-3xl font-black uppercase text-[12px] transition-all border-2 ${selectedAgencia?.id === ag.id ? 'bg-emerald-600 border-emerald-400 text-white shadow-lg scale-105' : 'bg-slate-800 border-slate-700 text-slate-400'}`}>
      ...
      <div className="max-w-[1600px] mx-auto grid lg:grid-cols-[1fr_450px] gap-8 px-6">
        ...
        <div className="space-y-10">
                   ...
                   {isVip ? (
                     <div className="space-y-6 animate-in zoom-in-95 duration-700">
                        ...
                        <div className="flex flex-wrap justify-center gap-3">
                           <span className="bg-emerald-500/20 text-emerald-400 px-5 py-2 rounded-2xl font-black text-sm uppercase italic tracking-widest">🎯 {datoVip?.metodo || "Matemática"}</span>
                        ...
                        <div className="mt-4 bg-white/5 p-4 rounded-2xl border border-white/10 text-left">
                          <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider leading-snug">
                             ...
                             {datoVip?.debug_suma || "Procesando malicia..."}
                          </p>
                        </div>
                     </div>
                   ) : (
          ...
          {/* 2. SELECTOR DE LOTERÍA (FIXED REJILLA PC) */}
          <Card className="bg-white p-6 lg:p-12 rounded-[3.5rem] shadow-xl border-none">
            <p className="text-[10px] font-black uppercase text-slate-400 mb-8 text-center tracking-[0.4em] italic">PASO 2: SELECCIONA LOTERÍA</p>
            <div className="grid grid-cols-3 lg:grid-cols-6 gap-6 lg:gap-10 items-center justify-items-center">
              {LOTERIAS.map(lot => (
                <button key={lot.id} onClick={() => { setSelectedLot(lot.id); setSelectedHours([]); setSelectedNum(null); }} className={`flex flex-col items-center gap-3 transition-all ${selectedLot === lot.id ? 'scale-110 opacity-100' : 'opacity-40 grayscale-0 hover:opacity-100 hover:scale-105'}`}>
        ...
        {/* COLUMNA DERECHA: TICKET (STICKY EN PC) */}
        <div className="space-y-8 flex flex-col items-center w-full">
          <div className="lg:sticky lg:top-32 space-y-8 w-full text-slate-900 text-center text-center">
            {selectedAgencia && (
              <Card className="p-8 bg-white rounded-[3rem] shadow-xl border-2 border-slate-100 flex flex-col gap-6 items-center">
                 ...
                 <div className="p-6 bg-slate-50 border-2 border-slate-200 rounded-[2rem] text-left w-full text-slate-900 text-center">
            ...
            <Card className="p-10 bg-white rounded-[4rem] shadow-2xl border-none space-y-4 text-center">
              <label className="text-[11px] font-black uppercase opacity-40 italic tracking-widest text-center text-slate-900 text-slate-900">MONTO POR SORTEO (BS)</label>
