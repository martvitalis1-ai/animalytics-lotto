...
export function ModuloJugadas() {
  ...
  const msgUrl = useMemo(() => {
    ...
    // MENSAJE COMPRIMIDO PARA EVITAR RECORTE
    ...
    currentJugadas.forEach(j => {
      ...
      msg += `Nro: ${j.numero} - ${j.animal}\n`;
      msg += `Hora: ${j.horas.join(", ")}\n`;
      msg += `Bs ${j.monto}\n`;
      msg += `----------\n`;
    });
  ...
  return (
    <div className="max-w-7xl mx-auto p-2 bg-slate-50 min-h-screen text-slate-900 pb-32">
      ...
      <div className="p-6 bg-slate-950 text-white shadow-2xl rounded-b-[2.5rem] mb-6">
        <p className="text-[10px] font-black uppercase text-emerald-400 mb-4 text-center tracking-[0.2em]">SELECCIONA TU AGENCIA</p>
        <div className="flex gap-4 overflow-x-auto pb-2 justify-center no-scrollbar">
          {agencias.map(ag => (
            <button key={ag.id} onClick={() => setSelectedAgencia(ag)} className={`flex items-center gap-3 px-8 py-4 rounded-2xl font-black uppercase text-[13px] transition-all border-2 ${selectedAgencia?.id === ag.id ? 'bg-emerald-600 border-emerald-400 shadow-lg' : 'bg-slate-900 border-slate-800 text-slate-500'}`}>
      ...
      {/* 2. LOTERIA SELECTOR (LOGOS GITHUB) */}
      <div className="p-4 bg-white shadow-sm mb-6 rounded-[2rem] mx-4 flex justify-center no-scrollbar">
        <div className="flex gap-8 min-w-max px-6 items-center">
          {LOTERIAS.map(lot => (
            <button key={lot.id} onClick={() => { setSelectedLot(lot.id); setSelectedHours([]); setSelectedNum(null); }} className={`flex flex-col items-center gap-2 transition-all ${selectedLot === lot.id ? 'scale-110 opacity-100' : 'opacity-30 grayscale'}`}>
      ...
      <div className="grid lg:grid-cols-3 gap-6 p-4 text-center">
        <div className="lg:col-span-2 space-y-6">
          <Card className="p-6 bg-white rounded-[2.5rem] shadow-xl border-none">
            <div className="grid grid-cols-5 md:grid-cols-8 gap-3">
              {Object.keys(animalitosAMostrar).map(n => (
                <button key={n} onClick={() => setSelectedNum(n)} className={`flex flex-col items-center p-3 rounded-[1.5rem] border-2 transition-all ${selectedNum === n ? 'border-emerald-500 bg-emerald-50 shadow-inner scale-105' : 'border-slate-50 bg-slate-50 text-slate-400'}`}>
                  <span className="text-3xl mb-1">{ANIMAL_EMOJIS[n] || '🎟️'}</span>
                  <span className="text-[12px] font-black text-slate-900">{n}</span>
                </button>
              ))}
            </div>
          </Card>
        ...
        <div className="space-y-6">
          ...
          {/* TICKET CON BOTÓN PREMIUM CENTRADO */}
          <div className="bg-white p-8 font-mono shadow-2xl rounded-[3rem] border-t-[20px] border-emerald-600 min-h-[500px] flex flex-col text-slate-900">
            ...
            {/* BOTÓN ENVIAR: ESTÉTICA PREMIUM IDÉNTICA AL DE ARRIBA */}
            <a 
              href={msgUrl} 
              ...
              className="w-full h-20 bg-emerald-600 hover:bg-emerald-700 text-white rounded-[2rem] font-black text-xl shadow-xl flex items-center justify-center text-center gap-4 transition-all hover:scale-[1.02] active:scale-95 leading-none"
            >
