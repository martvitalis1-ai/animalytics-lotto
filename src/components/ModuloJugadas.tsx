...
export function ModuloJugadas() {
  ...
  // ✅ CARGA EL DATO MAESTRO ESPECÍFICO POR LOTERÍA
  const cargarDatoIA = async () => {
    try {
      if (!selectedLot) return;
      setDatoVip(null); // Limpiar para evitar que se vea el anterior
  ...
  const validarVip = async () => {
    ...
    try {
      ...
      if (data) { setIsVip(true); localStorage.setItem('vip_active', 'true'); toast.success("¡BÚNKER VIP ACTIVADO!"); }
      else { toast.error("Código incorrecto"); }
    } catch (err) { toast.error("Fallo de red"); }
    ...
    <div className="w-full bg-[#F8FAFC] min-h-screen text-slate-900 pb-40 overflow-x-hidden text-center flex flex-col items-center">
      ...
      <div className="w-full bg-[#0F172A] p-6 lg:p-10 text-white shadow-2xl rounded-b-[3rem] mb-10">
        ...
        <div className="flex flex-wrap gap-4 justify-center">
          {agencias.map(ag => (
            <button key={ag.id} onClick={() => setSelectedAgencia(ag)} className={`flex items-center gap-2 px-8 py-4 rounded-3xl font-black uppercase text-[12px] border-2 transition-all ${selectedAgencia?.id === ag.id ? 'bg-emerald-600 border-emerald-400 text-white shadow-lg scale-105' : 'bg-slate-800 border-slate-700 text-slate-400'}`}>
      ...
      <div className="max-w-[1600px] w-full grid lg:grid-cols-[1fr_450px] gap-8 px-4 lg:px-10">
        <div className="space-y-10">
          ...
          <Card className="p-8 lg:p-12 bg-slate-900 border-none shadow-2xl rounded-[4rem] overflow-hidden relative border-t-[12px] border-emerald-500 text-center">
             ...
             <div className="relative z-10 space-y-8">
                <div className="flex justify-between items-center px-4 text-white">
                  ...
                  <span className="text-emerald-400 font-black text-2xl italic tracking-tighter uppercase underline">{selectedLot}</span>
                </div>
          ...
          <Card className="p-6 lg:p-12 bg-white rounded-[3.5rem] shadow-2xl border-none">
            <div className="grid grid-cols-4 md:grid-cols-6 lg:grid-cols-8 gap-3 text-slate-900">
