...
export function ModuloJugadas() {
  ...
  const whatsappUrl = useMemo(() => {
    ...
    return `https://wa.me/${phoneClean}?text=${encodeURIComponent(ticketMsg)}`;
  ...
  if (loading) return <div className="p-20 text-center animate-pulse font-black text-primary">Cargando Agencias...</div>;

  return (
    <div className="max-w-6xl mx-auto p-2 md:p-6 space-y-10 text-left">
      <section className="grid lg:grid-cols-2 gap-8">
        <div className="space-y-6">
          {/* DATOS DE COBRO USUARIO */}
          <div className="bg-primary/5 p-5 rounded-3xl border border-primary/10 space-y-4 shadow-inner">
            ...
            <div className="grid grid-cols-2 gap-2">
                <Input value={userPM} onChange={e => { setUserPM(e.target.value); localStorage.setItem('u_pm', e.target.value); }} placeholder="Tlf Pago Móvil" className="bg-background font-bold text-xs" />
        ...
        <div className="space-y-4">
          <div className="bg-white text-slate-900 p-8 font-mono shadow-2xl rounded-sm border-t-[14px] border-primary min-h-[450px] relative">
             ...
             {/* ✅ LA SOLUCIÓN DEFINITIVA: UN ENLACE <A> REAL QUE EL NAVEGADOR NO BLOQUEA */}
             <a 
               href={whatsappUrl} 
               target="_blank" 
               ...
               onClick={() => {
                 // Guardar ticket en historial antes de irse
                 ...
                 const h = JSON.parse(localStorage.getItem('tickets_history_v_final') || '[]');
                 localStorage.setItem('tickets_history_v_final', JSON.stringify([t, ...h]));
                 // Opcional: limpiar jugadas actuales
                 setCurrentJugadas([]);
