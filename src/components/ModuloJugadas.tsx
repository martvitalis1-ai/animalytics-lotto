...
export function ModuloJugadas() {
  ...
  // Datos usuario
  const [userBanco, setUserBanco] = useState(() => localStorage.getItem('u_banco_p') || "");
  const [userPM, setUserPM] = useState(() => localStorage.getItem('u_pm_p') || "");
  const [userCedula, setUserCedula] = useState(() => localStorage.getItem('u_cedula_p') || "");
  ...
  const agregarJugada = () => {
    ...
    let n = parseInt(num);
    let finalNum = "";
    if (num === "00") finalNum = "00";
    else if (n === 0) finalNum = "0";
    else {
      if (lot.includes("Guácharo") && n > 75) n = n % 76;
      if (!lot.includes("Guacharito") && !lot.includes("Guácharo") && n > 36) n = n % 37;
      finalNum = n < 10 ? "0" + n : n.toString();
    }

    setCurrentJugadas([...currentJugadas, { loteria: lot, numero: finalNum, monto: parseFloat(mon), horas: [...selectedHours].sort() }]);
    setNum(""); setMon(""); setSelectedHours([]);
  };

  // GENERADOR DE LINK (wa.me es el único que Opera no bloquea)
  const getWhatsappLink = () => {
    if (!selectedAgencia || currentJugadas.length === 0) return "#";
    
    let text = `*JUGADA - ANIMALYTICS PRO*\n----------------------------\n*MIS DATOS DE COBRO:*\n🏦 Banco: ${userBanco}\n📞 PM: ${userPM}\n🆔 CI: ${userCedula}\n----------------------------\n`;
    
    let totalTicket = 0;
    currentJugadas.forEach((j) => {
      text += `📍 *${j.loteria.toUpperCase()}*\n#${j.numero} - Sorteos: ${j.horas.join(", ")}\nMonto: ${j.monto} Bs c/u\n---\n`;
      totalTicket += (j.monto * j.horas.length);
    });

    text += `*TOTAL A PAGAR:* ${totalTicket.toFixed(2)} Bs\n----------------------------\n📥 _Envío captura de pago._`;
    
    const phone = selectedAgencia.whatsapp.toString().replace(/\D/g, '');
    return `https://wa.me/${phone}?text=${encodeURIComponent(text)}`;
  };

  if (loading) return <div className="p-20 text-center animate-pulse font-black text-primary">Cargando Búnker...</div>;
      ...
      <section className="grid lg:grid-cols-2 gap-8">
        <div className="space-y-6">
          <div className="bg-primary/5 p-5 rounded-3xl border border-primary/10 space-y-4 shadow-inner text-left">
            ...
            <Input value={userBanco} onChange={e => { setUserBanco(e.target.value); localStorage.setItem('u_banco_p', e.target.value); }} placeholder="Tu Banco" className="bg-background font-bold text-xs h-9" />
            <div className="grid grid-cols-2 gap-2">
                <Input value={userPM} onChange={e => { setUserPM(e.target.value); localStorage.setItem('u_pm_p', e.target.value); }} placeholder="Tlf Pago Móvil" className="bg-background font-bold text-xs h-9" />
                <Input value={userCedula} onChange={e => { setUserCedula(e.target.value); localStorage.setItem('u_cedula_p', e.target.value); }} placeholder="Tu Cédula" className="bg-background font-bold text-xs h-9" />
            </div>
          </div>
          ...
          {selectedAgencia && (
            <div className="p-4 bg-emerald-500/5 border-2 border-emerald-500/20 rounded-2xl text-[11px] font-black uppercase italic space-y-1 shadow-sm">
              <p className="text-emerald-600 text-[8px] uppercase">🏦 Pagar a Agencia (Pago Móvil):</p>
          ...
          <Card className="border-2 rounded-3xl border-primary/20 p-5 space-y-4 shadow-xl text-left">
            <Select onValueChange={setLot}><SelectTrigger className="font-black h-11 uppercase"><SelectValue placeholder="Elegir Lotería" /></SelectTrigger>
        ...
        <div className="space-y-4">
          <div className="bg-white text-slate-900 p-8 font-mono shadow-2xl rounded-sm border-t-[14px] border-primary min-h-[450px] relative text-left">
             <div className="text-center border-b-2 border-dashed pb-4 mb-6">
               ...
               <h3 className="font-black text-2xl uppercase tracking-tighter leading-none">{selectedAgencia?.nombre || "NUEVA JUGADA"}</h3>
               <p className="text-[9px] opacity-60 uppercase font-black mt-1">Animalytics Pro Ticket</p>
             </div>
             <div className="space-y-5">
               {currentJugadas.map((j, i) => (
                 <div key={i} className={`text-[12px] border-b border-dotted pb-3 flex justify-between items-center text-left`}>
                    ...
                    <button onClick={() => setCurrentJugadas(currentJugadas.filter((_, idx) => idx !== i))} className="text-red-600 p-1 ml-4"><Trash2 size={18}/></button>
                 </div>
               ))}
             ...
             {/* ✅ EL BOTÓN MAGICO: UN ENLACE <A> QUE OPERA NO BLOQUEA */}
             <a 
