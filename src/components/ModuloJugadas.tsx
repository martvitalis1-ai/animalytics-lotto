...
export function ModuloJugadas() {
  ...
  // ✅ CARGA DATO IA DINÁMICO POR SORTEO
  const cargarDatoIA = async () => {
    if (!selectedLot) return;
    setIaLoading(true);
    setDatoVip(null); 

    // Calculamos la hora real del próximo sorteo para enviarla al SQL
    const ahora = new Date();
    let horaSorteo = ahora.getHours();
    if (ahora.getMinutes() >= 0) horaSorteo += 1;
    const ampm = horaSorteo >= 12 ? 'PM' : 'AM';
    const horaFormateada = `${horaSorteo > 12 ? horaSorteo - 12 : (horaSorteo === 0 ? 12 : horaSorteo)}:00 ${ampm}`;
    
    try {
      const { data, error } = await supabase.rpc('generar_dato_maestro_vip', {
        lot_name: selectedLot, 
        proxima_hora: horaFormateada
      });
      if (data && data.length > 0) setDatoVip(data[0]);
    } catch (e) { 
        console.warn("IA no respondió");
