...
export function ModuloJugadas() {
  ...
  const cargarDatoIA = async () => {
    ...
    try {
      ...
      else setDatoVip({ animal_id: "04", animal_nombre: "ALACRÁN", probabilidad: 92, metodo: "Tendencia Semanal" });
    } catch (e) { 
          ...
          <Card className="p-8 lg:p-12 bg-slate-900 border-none shadow-2xl rounded-[4rem] overflow-hidden relative border-t-[12px] border-emerald-500">
             ...
             <div className="relative z-10 space-y-8 text-white">
                <div className="flex justify-between items-center px-4">
                  ...
                  <span className="text-emerald-400 font-black text-2xl italic tracking-tighter uppercase underline decoration-2">{LOTERIAS.find(l => l.id === selectedLot)?.label || "LOTERIA"}</span>
                </div>
                     ...
                     iaLoading ? (
                        ...
                        <div className="space-y-6 animate-in zoom-in-95 duration-700">
                           ...
                           <div className="flex flex-wrap justify-center gap-3">
                              <span className="bg-emerald-500/20 text-emerald-400 px-5 py-2 rounded-2xl font-black text-xs uppercase italic">🎯 {datoVip?.metodo}</span>
                              <span className="bg-white/10 text-white px-5 py-2 rounded-2xl font-black text-xs uppercase italic">🔥 {datoVip?.probabilidad || "0"}% ÉXITO</span>
                           </div>
                        </div>
                     )
                   ) : (
