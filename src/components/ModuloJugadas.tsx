...
export function ModuloJugadas() {
  ...
  // --- FUNCIÓN DE ENVÍO RECONSTRUIDA DESDE CERO ---
  const handleSendWhatsApp = () => {
    ...
    // 2. Mensaje sin NINGÚN símbolo especial (Texto puro para evitar recortes)
    let msg = "VERSION-BUNKER-FINAL\n";
    msg += "SOLICITUD DE JUGADA\n";
    msg += "--------------------------\n";
    msg += "DATOS DE PAGO:\n";
    msg += "BANCO: " + userBanco + "\n";
    msg += "TELEFONO: " + userPM + "\n";
    ...
    // 4. Codificación manual ultra-segura
    const encodedText = encodeURIComponent(msg);
    
    // 5. Usamos el protocolo 'whatsapp://' que fuerza la apertura de la APP en el celular
    // Si falla, usamos el protocolo web como respaldo.
    const whatsappIntent = `whatsapp://send?phone=${tlf}&text=${encodedText}`;
    const whatsappWeb = `https://api.whatsapp.com/send?phone=${tlf}&text=${encodedText}`;

    // Intentar abrir la app directamente
    window.location.href = whatsappIntent;

    // Si en 500ms no ha salido de la página, es que no reconoció el protocolo, usamos web
    setTimeout(() => {
        if (!document.hidden) {
      ...
      <section className="grid lg:grid-cols-2 gap-8">
        <div className="space-y-6">
          <div className="bg-primary/5 p-5 rounded-3xl border border-primary/10 space-y-4">
            <label className="text-[10px] font-black uppercase text-primary">1. Datos de Cobro</label>
          ...
          <div className="space-y-2">
            ...
            <div className="flex flex-wrap gap-2">
              {agencias.map(ag => (
                <button key={ag.id} onClick={() => setSelectedAgencia(ag)} className={`px-3 py-2 border-2 rounded-xl text-[10px] font-black uppercase transition-all ${selectedAgencia?.id === ag.id ? 'border-primary bg-primary/10' : 'bg-card'}`}>
                  {ag.nombre}
                </button>
              ))}
            </div>
          ...
          <Card className={`border-2 rounded-3xl p-5 space-y-4 shadow-xl ${editingIndex !== null ? 'border-amber-500 bg-amber-50' : 'border-primary/20'}`}>
            <Select value={lot} onValueChange={setLot}>
              <SelectTrigger className="font-black uppercase"><SelectValue placeholder="Lotería" /></SelectTrigger>
              <SelectContent>
            ...
            <div className="grid grid-cols-2 gap-2">
              <Input placeholder="Nº" value={num} onChange={e => setNum(e.target.value)} className="font-black text-center text-xl" />
              <Input placeholder="Bs" type="number" value={mon} onChange={e => setMon(e.target.value)} className="font-black text-center text-xl" />
            </div>
        ...
        <div className="space-y-4">
          <div className="bg-white text-slate-900 p-8 font-mono shadow-2xl rounded-sm border-t-[14px] border-primary relative">
             <div className="text-center border-b-2 border-dashed pb-4 mb-6">
               <h3 className="font-black text-2xl uppercase">{selectedAgencia?.nombre || "NUEVA JUGADA"}</h3>
             </div>
               ...
               {currentJugadas.map((j, i) => (
                 <div key={i} className="text-[12px] border-b border-dotted pb-3 flex justify-between items-center">
                    <div className="flex-1 text-left">
                      <div className="flex justify-between font-black uppercase text-primary"><span>{j.loteria}</span><span>#{j.numero}</span></div>
                      <p className="text-[9px] opacity-70">Sorteos: {j.horas.join(", ")}</p>
                    </div>
                    <div className="flex items-center gap-3 ml-4">
                       <span className="font-black text-sm">{(j.monto * j.horas.length).toFixed(2)}</span>
             ...
             <div className="mt-8 flex justify-between font-black text-xl border-t-2 pt-4 italic uppercase">
               <span>TOTAL:</span>
             ...
             <Button 
               ...
               className="w-full h-16 bg-[#25D366] hover:bg-[#20ba5a] text-white mt-10 rounded-2xl font-black text-lg shadow-2xl flex items-center justify-center gap-3"
             >
