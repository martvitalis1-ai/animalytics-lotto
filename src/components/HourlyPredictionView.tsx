...
import { Clock, ShieldCheck, Zap, Calendar } from "lucide-react";
...
export function HourlyPredictionView({ lotteryId }: { lotteryId: string }) {
  ...
  useEffect(() => {
    // 🛡️ LÓGICA DE HORARIOS SEGÚN TU SQL
    ...
    async function loadFullIntelligence() {
      // 🛡️ MAPEADO SEGÚN TU RESULTADO SQL #3 (Evita secciones vacías)
      let dbId = lotteryId.toLowerCase().trim();
      if (dbId === 'la_granjita') dbId = 'granjita';
      if (dbId === 'el_guacharo') dbId = 'guacharo';

      // 1. Cargar Historial para cálculos de presión térmica
      ...
      // 2. Cargar Pronóstico Maestro desde tu tabla SQL #2
      const { data: preds } = await supabase
        .from('super_pronostico_final')
        .select('pronostico_fijo, pronostico_jaladera')
        .eq('lottery_type', dbId)
        .order('ultimo_animal', { ascending: false }) // O por fecha si tienes la columna
        .limit(1)
        .maybeSingle();
    ...
    // 🛡️ ACTUALIZACIÓN EN TIEMPO REAL DESDE SUPABASE
    const channel = supabase.channel('ia-global-sync')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'lottery_results' }, () => loadFullIntelligence())
      .on('postgres_changes', { event: '*', schema: 'public', table: 'super_pronostico_final' }, () => loadFullIntelligence())
      .subscribe();
  ...
  const study = useMemo(() => {
    ...
    // Cerebro de Frecuencias (Estadística de Búnker)
    const freq: any = {};
    ...
    // 🛡️ ASIGNACIÓN SEGÚN TU TABLA "super_pronostico_final"
  ...
  if (!study) return (
    <div className="p-20 text-center flex flex-col items-center gap-4">
      ...
      <p className="font-black text-slate-400 uppercase italic">Sincronizando Cerebro con Supabase...</p>
    </div>
  );
  ...
  return (
    <div className="space-y-12 pb-40 px-1 animate-in fade-in duration-700">
      ...
      {/* 🛡️ 1. ESTUDIO PRÓXIMO SORTEO: DATOS DE TU TABLA MAESTRA */}
      <div className="bg-white border-4 border-slate-900 rounded-[3rem] md:rounded-[5rem] p-8 md:p-14 flex flex-col items-center shadow-2xl relative overflow-hidden">
        ...
        <div className="flex justify-center gap-4 md:gap-12 z-10">
           {study.maestros.map((code, i) => (
             <img key={`${code}-${i}`} src={getAnimalImageUrl(code)} className="w-[145px] h-[145px] md:w-[400px] md:h-[400px] object-contain drop-shadow-2xl" />
           ))}
        </div>
      ...
      {/* 2. TOP 3 DEL DÍA (GIGANTE Y CENTRADO) */}
      ...
      <div className="bg-yellow-500/10 p-8 rounded-[3rem] border-4 border-yellow-500/20 relative overflow-hidden shadow-xl text-center">
         ...
         <div className="grid grid-cols-3 md:grid-cols-3 gap-6 relative z-10">
            {study.enjaulados.map(c => <img key={c} src={getAnimalImageUrl(c)} className="w-32 h-32 md:w-52 md:h-52 object-contain mx-auto drop-shadow-md" />)}
         </div>
      </div>
        ...
        <div className="bg-white border-4 border-slate-900 rounded-[3rem] p-8 shadow-xl">
           ...
           <div className="space-y-4">{study.hours.map(h => (
             <div key={h.h} className="flex justify-between items-center p-6 bg-slate-50 rounded-3xl border-2 border-slate-100 shadow-inner">
                <span className="font-black text-lg text-slate-900">{h.h}</span>
        ...
        <div className="bg-white border-4 border-slate-900 rounded-[3rem] p-8 shadow-xl">
           ...
           <div className="space-y-4">{study.days.map(d => (
             <div key={d.d} className="flex justify-between items-center p-6 bg-slate-50 rounded-3xl border-2 border-slate-100 shadow-inner">
                <span className="font-black text-lg text-slate-900">{d.d}</span>
      ...
      {/* 7. RECOMENDACIÓN FINAL (2x2 EN MÓVIL) */}
         ...
         <div className="flex flex-col gap-10">
            ...
            <div className="bg-slate-900 text-white p-8 rounded-[2rem] border-l-8 border-emerald-500 shadow-xl">
               <p className="font-black text-sm md:text-xl uppercase leading-relaxed italic text-center">
                 EL ALGORITMO HA SINCRONIZADO LOS PRONÓSTICOS FIJOS Y DE JALADERA SEGÚN EL ÚLTIMO SORTEO DE {lotteryId.replace('_', ' ').toUpperCase()}.
               </p>
            </div>
         </div>
      </div>
