...
export function HourlyPredictionView({ lotteryId }: { lotteryId: string }) {
  ...
  useEffect(() => {
    const updateTime = () => {
      ...
      // REINICIO 8 PM (VIDEO 3)
      if (hour >= 20) { setNextHour("09:00 AM (Estudio Nuevo)"); }
      else {
    ...
    async function load() {
      // 🛡️ SINCRONIZACIÓN REAL (LIMITADO PARA EVITAR ERROR 429)
      const { data } = await supabase.from('lottery_results').select('*').eq('lottery_type', lotteryId).order('draw_date', { ascending: false }).limit(50);
      setResults(data || []);
    }
    load();
  }, [lotteryId]);
  ...
  const study = useMemo(() => {
    ...
    return { maestro: sorted[0][0], top3: [sorted[1][0], sorted[0][0], sorted[2][0]], hot: sorted.slice(1, 5).map(x => x[0]), cold: sorted.slice(-4).map(x => x[0]) };
  }, [results, lotteryId]);
  ...
  return (
    <div className="space-y-12 animate-in fade-in duration-700 pb-20">
      ...
      {/* BLOQUES CALIENTES Y FRÍOS (FOTO 1) */}
