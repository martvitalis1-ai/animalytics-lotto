    ...
    <div className="space-y-10 animate-in fade-in duration-700">
      ...
      <div className="bg-slate-900 text-white p-8 rounded-[4rem] border-b-8 border-emerald-500 shadow-2xl relative overflow-hidden">
        ...
        <div className="flex flex-col md:flex-row items-center gap-8 relative z-10">
          <div className="bg-emerald-500 p-5 rounded-[2.5rem] shadow-lg"><Zap size={40} className="fill-white" /></div>
          <div>
            <h2 className="text-3xl font-black italic uppercase tracking-tighter leading-none">ANIMAL MAESTRO: {data?.maestro}</h2>
            <p className="text-emerald-400 font-bold mt-2 uppercase text-[10px] tracking-widest italic">Ciclo de 72h detectado en {lotteryId}</p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white border-4 border-slate-900 rounded-[5rem] p-12 flex flex-col items-center shadow-[12px_12px_0px_0px_rgba(0,0,0,1)]">
        <div className="bg-slate-900 text-white px-8 py-2 rounded-full font-black uppercase text-[10px] italic tracking-[0.2em] mb-10">Próximo Sorteo Sugerido</div>
        <img src={getAnimalImageUrl(data?.maestro)} className="w-80 h-80 md:w-[450px] md:h-[450px] object-contain drop-shadow-[0_35px_35px_rgba(0,0,0,0.3)]" />
         ...
         <div className="bg-white border-4 border-slate-900 rounded-[4rem] p-8 shadow-[8px_8px_0px_0px_rgba(0,0,0,1)]">
            ...
            <div className="grid grid-cols-4 gap-4">
               {data?.hot.map((a: any, i: number) => (
                  <div key={i} className="flex flex-col items-center bg-slate-50 p-2 rounded-3xl border border-slate-100">
                    <img src={getAnimalImageUrl(a.result_number)} className="w-16 h-16 object-contain" />
                    <span className="text-[10px] font-black mt-2 text-slate-400">#{a.result_number}</span>
                  </div>
               ))}
            </div>
         </div>
