...
import { Clock, Calendar, ShieldCheck, Zap, Star, Snowflake, Lock } from "lucide-react";
...
export function HourlyPredictionView({ lotteryId }: { lotteryId: string }) {
  ...
  useEffect(() => {
    ...
    async function loadFullIntelligence() {
      ...
      const { data: history } = await supabase.from('lottery_results').select('result_number').eq('lottery_type', dbId).order('draw_date', { ascending: false }).limit(400);
      setResults(history || []);
  ...
  if (!study) return <div className="p-20 text-center font-black text-emerald-500 uppercase animate-pulse">Sincronizando Búnker...</div>;
    ...
    <div className="space-y-12 pb-40 px-1 animate-in fade-in duration-700">
      ...
      <div className="space-y-8">
        ...
        <div className="bg-yellow-500/10 p-8 rounded-[3rem] border-4 border-yellow-500/20 relative overflow-hidden shadow-xl text-center">
           ...
           <div className="grid grid-cols-3 md:grid-cols-3 gap-6 relative z-10">{study.enjaulados.map(c => <img key={c} src={getAnimalImageUrl(c)} className="w-24 h-24 md:w-48 md:h-48 object-contain mx-auto drop-shadow-md" />)}</div>
        </div>
      </div>

      {/* 4. MEJORES HORAS Y DÍAS (RESTAURADOS) */}
