...
export function HourlyPredictionView({ lotteryId }: { lotteryId: string }) {
  ...
  useEffect(() => {
    const updateTimeLogic = () => {
      ...
      if (hour >= 20) {
        setNextHour("09:00 AM (Nuevo Estudio)");
      } else {
    ...
    async function load() {
      // SINCRONIZACIÓN TOTAL: Usamos el ID de la base de datos real
  ...
  if (!study) return <div className="p-20 text-center font-black animate-pulse text-emerald-500">SINCRONIZANDO BÚNKER...</div>;
