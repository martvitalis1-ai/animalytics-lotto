...
export function HourlyPredictionView({ lotteryId }: { lotteryId: string }) {
  ...
  useEffect(() => {
    const updateTimeLogic = () => {
      ...
      // REINICIO DE ESTUDIO (VIDEO): A las 8 PM (20:00) se prepara el estudio de mañana
      if (hour >= 20) {
        setNextHour("09:00 AM (Estudio Nuevo)");
      } else {
    ...
    async function load() {
      // SINCRONIZACIÓN TOTAL: Filtro dinámico por lotería (Granjita, Rey, etc.)
  ...
  if (!study) return <div className="p-20 text-center font-black animate-pulse text-emerald-500 uppercase">SINCRONIZANDO HISTORIAL DE {lotteryId.toUpperCase()}...</div>;
        ...
        <div className="flex justify-around items-end gap-4">
           {study.top3.map((code:any, i:number) => (
             <div key={code} className="flex flex-col items-center">
                <img src={getAnimalImageUrl(code)} className={`${i===1?'w-56 h-56':'w-28 h-28'} object-contain drop-shadow-lg`} />
