...
export function HourlyPredictionView({ lotteryId }: { lotteryId: string }) {
  ...
  useEffect(() => {
    async function load() {
      ...
      if (res && res.length > 0) {
        // Lógica simple para mostrar datos reales
  ...
  if (loading) return <div className="p-20 text-center font-black animate-pulse text-emerald-500">CONECTANDO AL BÚNKER...</div>;

  return (
    <div className="space-y-10 animate-in fade-in zoom-in duration-700">
      {/* HEADER ANIMAL MAESTRO */}
      <div className="bg-slate-900 text-white p-8 rounded-[4rem] border-b-8 border-emerald-500 shadow-2xl relative overflow-hidden">
        ...
        <div className="flex flex-col md:flex-row items-center gap-8 relative z-10">
          ...
          <div>
            ...
            <p className="text-emerald-400 font-bold mt-2 uppercase text-xs tracking-widest italic">IA Detecta Presión Térmica en {lotteryId}</p>
          </div>
        </div>
      </div>

      {/* ANIMAL GIGANTE (FOTO 1 DEL VIDEO) */}
      <div className="bg-white border-4 border-slate-900 rounded-[5rem] p-12 flex flex-col items-center shadow-[12px_12px_0px_0px_rgba(0,0,0,1)]">
        <div className="bg-slate-900 text-white px-8 py-2 rounded-full font-black uppercase text-xs italic tracking-[0.2em] mb-8">Próximo Sorteo</div>
      ...
      {/* BLOQUES TÉRMICOS (FOTO 1 DEL VIDEO) */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
         <div className="bg-white border-4 border-slate-900 rounded-[4rem] p-8 shadow-[8px_8px_0px_0px_rgba(0,0,0,1)]">
            <h4 className="font-black uppercase text-xs text-orange-500 border-b-2 pb-2 italic mb-6">🔥 Números Calientes</h4>
         ...
         <div className="bg-white border-4 border-slate-900 rounded-[4rem] p-8 shadow-[8px_8px_0px_0px_rgba(0,0,0,1)]">
            <h4 className="font-black uppercase text-xs text-blue-500 border-b-2 pb-2 italic mb-6">❄️ Números Fríos</h4>
            <div className="grid grid-cols-4 gap-4">
