...
export function HourlyPredictionView({ lotteryId }: { lotteryId: string }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      // 🛡️ CORRECCIÓN DE IDS: la_granjita y lotto_rey para que cargue historial
      const dbId = lotteryId === 'granjita' ? 'la_granjita' : lotteryId === 'lotto_rey' ? 'lotto_rey' : lotteryId;
      const { data } = await supabase.from('lottery_results').select('*').eq('lottery_type', dbId).order('draw_date', { ascending: false }).limit(200);
      setResults(data || []);
    }
    load();
  }, [lotteryId]);
  ...
  if (!study) return <div className="p-20 text-center font-black animate-pulse text-emerald-500 uppercase">SINCRONIZANDO HISTORIAL...</div>;
  ...
  return (
    <div className="space-y-12 animate-in fade-in duration-700">
      ...
      {/* TOP 3 SEGÚN ESTUDIO (COMO EL VIDEO) */}
        ...
        <div className="flex justify-around items-end gap-4">
           {study.top3.map((code:any, i:number) => (
             <div key={code} className="flex flex-col items-center">
                ...
                <span className="font-black text-xl mt-4">#{code}</span>
             </div>
           ))}
        </div>
      </div>
