...
// 🛡️ IMPORTACIÓN CORREGIDA: Agregado 'Calendar' que faltaba
...
export function HourlyPredictionView({ lotteryId }: { lotteryId: string }) {
  ...
  const study = useMemo(() => {
    ...
    const hot = fullSortedList.filter(c => !used.has(c)).slice(0, 5);
    const frios = [...fullSortedList].reverse().filter(c => !used.has(c)).slice(0, 5);
    const enjaulados = [...fullSortedList].reverse().filter(c => !used.has(c) && !frios.includes(c)).slice(0, 3);
    ...
    return {
      ...
      days: [
        { d: "LUNES", p: [fullSortedList[14], fullSortedList[15]] }, 
        { d: "VIERNES", p: [fullSortedList[16], fullSortedList[17]] }
      ],
    };
    ...
    <div className="space-y-12 pb-40 px-1 animate-in fade-in duration-700">
      ...
      <div className="space-y-8">
        ...
        <div className="bg-yellow-500/10 p-8 rounded-[3rem] border-4 border-yellow-500/20 relative overflow-hidden shadow-xl text-center">
           <h4 className="font-black text-2xl uppercase italic mb-8 border-b-4 border-yellow-500 pb-2 text-yellow-700 uppercase">⏳ ENJAULADOS (VENCIDOS)</h4>
           ...
           <div className="space-y-4">
             {study.hours.map(h => (
               <div key={h.h} className="flex justify-between items-center p-6 bg-slate-50 rounded-3xl border-2 border-slate-100 shadow-inner">
                  <span className="font-black text-lg text-slate-900 uppercase">{h.h}</span>
           ...
           <div className="space-y-4">
             {study.days.map(d => (
               <div key={d.d} className="flex justify-between items-center p-6 bg-slate-50 rounded-3xl border-2 border-slate-100 shadow-inner">
                  <span className="font-black text-lg text-slate-900 uppercase">{d.d}</span>
