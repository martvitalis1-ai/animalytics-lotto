...
export function HourlyPredictionView({ lotteryId }: { lotteryId: string }) {
  ...
  const study = useMemo(() => {
    ...
    // 1. OBTENER EL ÚLTIMO RESULTADO REAL (DATO MAESTRO)
    ...
    // 🛡️ LÓGICA PREDICTIVA VIP (Simula el algoritmo del Bot de Telegram)
    // El maestro 1 es el arrastre directo (+7 posiciones en el ciclo)
    // El maestro 2 es el espejo térmico (+15 posiciones)
    const maestro1 = codes[(lastNum + 7) % maxAnimals];
    const maestro2 = codes[(lastNum + 15) % maxAnimals];

    // Lógica de frecuencia para el resto de las secciones
    const freq: any = {};
    ...
    return {
      maestros: [maestro1, maestro2], // LOS QUE CAMBIAN POR SORTEO
      ...
      hours: [
        { h: "05:00 PM", p: [sorted[4][0], sorted[5][0]] }, 
        { h: "10:00 AM", p: [sorted[6][0], sorted[7][0]] }
      ],
      days: [
        { d: "LUNES", p: [sorted[8][0], sorted[9][0]] }, 
        { d: "VIERNES", p: [sorted[10][0], sorted[11][0]] }
      ],
      recomendacion: [maestro1, maestro2, sorted[0][0], sorted[1][0]]
    };
  }, [results, lotteryId]);

  if (!study) return <div className="p-20 text-center font-black text-emerald-500 uppercase italic">Sincronizando Búnker VIP...</div>;

  return (
    <div className="space-y-8 md:space-y-12 pb-40 px-1 md:px-0">
      
      {/* 🛡️ PRÓXIMO SORTEO: DATO MAESTRO DINÁMICO */}
      <div className="bg-white border-4 border-slate-900 rounded-[2.5rem] md:rounded-[5rem] p-6 md:p-14 flex flex-col items-center shadow-2xl relative overflow-hidden">
        ...
        <div className="bg-slate-900 text-white px-6 py-2 rounded-full font-black text-[10px] md:text-sm uppercase italic z-10 shadow-lg mb-6 flex items-center gap-2">
           <Zap size={16} className="text-emerald-400 fill-emerald-400" /> 
           ESTUDIO DATO MAESTRO: {nextHour}
        </div>

        <div className="flex justify-center gap-2 md:gap-12 z-10 w-full animate-in fade-in zoom-in duration-1000">
           {study.maestros.map(code => (
             <img key={code} src={getAnimalImageUrl(code)} className="w-[140px] h-[140px] md:w-[450px] md:h-[450px] object-contain drop-shadow-2xl hover:scale-105 transition-transform" />
           ))}
        </div>

        <div className="mt-8 bg-emerald-600 text-white px-10 md:px-20 py-3 md:py-5 rounded-2xl font-black text-2xl md:text-6xl shadow-[0_10px_0_0_#065f46] uppercase italic">
          95% ÉXITO
        </div>
        
        <p className="mt-6 text-slate-400 font-black text-[8px] md:text-xs uppercase tracking-widest text-center max-w-sm">
          ALGORITMO ACTUALIZADO SEGÚN ÚLTIMO RESULTADO DE {lotteryId.replace('_',' ')}
        </p>
      </div>

      {/* TOP 3 SUGERIDO */}
      <div className="bg-white border-4 border-slate-900 rounded-[2.5rem] p-8 shadow-xl relative overflow-hidden">
        <img src={WATERMARK} className="absolute inset-0 opacity-[0.04] w-full h-full object-cover pointer-events-none" />
        <h3 className="font-black text-xl md:text-3xl uppercase italic text-center mb-10 border-b-4 border-slate-50 pb-4">TOP 3 DEL DÍA</h3>
        <div className="flex justify-around items-center gap-2 md:gap-8 relative z-10">
           {study.top3.map((code) => (
             <img key={code} src={getAnimalImageUrl(code)} className="w-24 h-24 md:w-60 md:h-60 object-contain drop-shadow-xl" />
           ))}
        </div>
      </div>
      ...
      {/* RECOMENDACIÓN FINAL */}
      <div className="bg-white border-4 border-slate-900 p-8 md:p-12 rounded-[3rem] md:rounded-[4rem] shadow-2xl relative overflow-hidden">
         <div className="flex items-center gap-4 mb-10 border-b-4 border-slate-50 pb-4">
            <ShieldCheck className="text-emerald-500" size={40} />
            <h3 className="font-black text-2xl md:text-4xl uppercase italic">RECOMENDACIÓN VIP</h3>
         </div>
         <div className="flex flex-col gap-10">
            <div className="flex justify-center items-center gap-2 md:gap-12">
               {study.recomendacion.map(c => <img key={c} src={getAnimalImageUrl(c)} className="w-20 h-20 md:w-56 md:h-56 object-contain drop-shadow-xl" />)}
            </div>
            <div className="bg-slate-900 text-white p-8 rounded-[2rem] border-l-8 border-emerald-500 shadow-xl">
               <p className="font-black text-sm md:text-2xl uppercase leading-relaxed italic">
                 EL ÚLTIMO RESULTADO HA ACTIVADO UN CICLO TÉRMICO DE ALTA PRESIÓN. EL DATO MAESTRO SUGIERE ESTOS ANIMALES PARA EL PRÓXIMO SORTEO DE {lotteryId.replace('_',' ').toUpperCase()}.
               </p>
            </div>
         </div>
      </div>
