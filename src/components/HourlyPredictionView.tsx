...
export function HourlyPredictionView({ lotteryId }: { lotteryId: string }) {
  ...
  useEffect(() => {
    const updateStudy = () => {
      ...
      if (hour >= 20) { setNextHour("08:00 AM (Mañana)"); }
      else {
    ...
    async function load() {
      // 🛡️ ABSORCIÓN REAL (LIMITADO PARA EVITAR ERROR 429)
  ...
  return (
    <div className="space-y-12 animate-in fade-in duration-700 pb-20">
      ...
      <div className="bg-white border-4 border-slate-900 rounded-[5rem] p-12 flex flex-col items-center shadow-2xl relative overflow-hidden">
        ...
        <img src={getAnimalImageUrl(study.maestro)} className="w-[300px] h-[300px] md:w-[480px] md:h-[480px] object-contain mt-8 z-10" />
      ...
      <div className="bg-white border-4 border-slate-900 rounded-[4rem] p-10 shadow-xl text-center">
        <h3 className="font-black text-2xl uppercase italic mb-12 border-b pb-4">Top 3 Sugerido para Hoy</h3>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        <div className="bg-slate-50 border-4 border-slate-900 rounded-[3.5rem] p-8 shadow-xl">
          <h4 className="font-black text-orange-500 uppercase italic mb-8 border-b-2 pb-2">🔥 Números Calientes</h4>
          <div className="grid grid-cols-3 gap-6">{study.hot.map(c => <img key={c} src={getAnimalImageUrl(c)} className="w-24 h-24 object-contain" />)}</div>
        </div>
        <div className="bg-slate-50 border-4 border-slate-900 rounded-[3.5rem] p-8 shadow-xl">
          <h4 className="font-black text-blue-500 uppercase italic mb-8 border-b-2 pb-2">❄️ Números Fríos</h4>
          <div className="grid grid-cols-3 gap-6">{study.cold.map(c => <img key={c} src={getAnimalImageUrl(c)} className="w-24 h-24 object-contain" />)}</div>
        </div>
      </div>
    </div>
  );
