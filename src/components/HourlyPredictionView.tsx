...
export function HourlyPredictionView({ lotteryId }: { lotteryId: string }) {
  ...
  const getLotteryStudy = (id: string) => {
    ...
    // 🛡️ EXPLOSIVOS = Top 1, 2, 3 de frecuencia
    // 🛡️ TOP 3 DEL DÍA = Top 4, 5, 6 (Animales calientes pero diferentes)
    return {
      maestro: sorted[0][0],
  ...
  return (
    <div className="space-y-16 animate-in fade-in duration-700 pb-40">
      ...
      <div className="bg-white border-4 border-slate-900 rounded-[5rem] p-12 flex flex-col items-center shadow-2xl relative overflow-hidden">
        ...
        <div className="bg-slate-900 text-white px-10 py-2 rounded-full font-black text-[12px] uppercase italic z-10 shadow-lg mb-4 text-center">ANIMAL MAESTRO DEL DÍA</div>
        ...
        <div className="mt-10 bg-emerald-600 text-white px-16 py-5 rounded-[2.5rem] font-black text-4xl shadow-xl border-b-8 border-emerald-800 animate-bounce z-10">95% ÉXITO</div>
      </div>

      {/* 2. REJILLA DE LAS 6 LOTERÍAS (ESTUDIO BLINDADO) */}
        ...
        {LOTTERIES.map((lot) => {
          ...
          return (
            <div key={lot.id} className="bg-white border-4 border-slate-900 rounded-[5rem] p-10 shadow-[12px_12px_0px_0px_rgba(0,0,0,1)] flex flex-col items-center">
              <div className="bg-slate-900 text-white px-8 py-2 rounded-full font-black text-xs uppercase mb-12 tracking-[0.2em] italic shadow-md">{lot.name}</div>
              
              <p className="text-emerald-500 font-black text-xs uppercase tracking-widest mb-10 italic">TOP 3 DEL DÍA</p>
              
              <div className="flex justify-center items-center gap-2 md:gap-6 w-full">
                {study.top3.map((code, index) => (
                  <div key={index} className="flex-1 flex justify-center">
                     {/* ANIMALES DEL MISMO TAMAÑO Y GIGANTES (w-44) */}
                     <img 
