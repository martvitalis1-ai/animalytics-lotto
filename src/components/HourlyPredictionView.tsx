  ...
  return (
    <div className="space-y-12 animate-in fade-in duration-700 pb-20">
      <div className="bg-white border-4 border-slate-900 rounded-[5rem] p-12 flex flex-col items-center shadow-2xl relative overflow-hidden">
        ...
        <div className="mt-8 bg-emerald-600 text-white px-12 py-4 rounded-3xl font-black text-4xl shadow-xl border-b-8 border-emerald-800 animate-bounce z-10 border-4 border-slate-900">95% ÉXITO</div>
        <p className="mt-8 text-slate-400 font-black italic uppercase text-center max-w-md z-10 text-sm">PATRÓN DETECTADO: Ciclo térmico de arrastre de 72h.</p>
      </div>
        ...
        <div className="flex justify-around items-end gap-4">
           {study.top3.map((code:any, i:number) => (
             <div key={code} className="flex flex-col items-center">
                ...
                <span className="font-black text-lg mt-4 text-slate-900">#{code}</span>
             </div>
           ))}
        </div>
      </div>
