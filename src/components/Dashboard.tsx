...
export function Dashboard({ userRole, onLogout }: any) {
  ...
  return (
    <div className="min-h-screen bg-white text-slate-900 font-sans antialiased">
      ...
      {/* BARRA DE SELECTOR SÓLIDO (FIX VIDEO) */}
      <div className="bg-slate-100 border-b-2 border-slate-900 p-4 sticky top-[80px] z-40">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-4">
          <div className="bg-white border-2 border-slate-900 rounded-xl p-1 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
            <Select value={globalLottery} onValueChange={setGlobalLottery}>
              <SelectTrigger className="w-64 h-10 border-none font-black uppercase text-xs">
                <SelectValue />
              </SelectTrigger>
          ...
          <TabsList className="bg-white p-1 rounded-full h-14 border-2 border-slate-900 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] flex overflow-x-auto no-scrollbar w-full">
            {["ia", "explosivo", "deportes", "ruleta", "resultados", "matriz", "guia", "admin"].map((tab) => (
              <TabsTrigger key={tab} value={tab} onClick={() => setActiveTab(tab)} className="rounded-full px-5 font-black text-[10px] uppercase data-[state=active]:bg-emerald-500 data-[state=active]:text-white flex-1">{tab}</TabsTrigger>
            ))}
          </TabsList>
        </div>
      </div>
      ...
      <main className="p-4 max-w-7xl mx-auto pb-20">
        <Tabs value={activeTab} className="space-y-8">
          ...
          <TabsContent value="deportes"><div className="p-20 text-center border-4 border-slate-900 rounded-[3rem] bg-white shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] font-black uppercase italic">Sincronizando Líneas de Las Vegas...</div></TabsContent>
        </Tabs>
      </main>
    </div>
  );
