...
import { CombinationsView } from "./CombinationsView"; // 🛡️ Archivo que acabamos de crear
        ...
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-4">
          <div className="flex items-center shrink-0">
             ...
             <div className="flex flex-col ml-8 md:ml-16">
                ...
                <span className="text-[9px] md:text-[16px] font-black text-emerald-300 uppercase tracking-[0.35em] mt-1.5 drop-shadow-md">Bunker Intelligence</span>
             </div>
          </div>
          <div className="flex items-center gap-3 flex-1 justify-end">
            <div className="bg-white rounded-xl border-2 border-emerald-500 shadow-md flex-1 md:flex-none">
              <Select value={globalLottery} onValueChange={setGlobalLottery}>
                <SelectTrigger className="w-full md:w-[240px] h-10 border-none bg-transparent font-black uppercase text-[10px] md:text-xs text-slate-900 focus:ring-0 px-4"><SelectValue /></SelectTrigger>
            ...
            <Button onClick={() => window.open('https://t.me/+BXV4GahQ4gswNmNh', '_blank')} className="bg-[#229ED9] h-10 px-3 rounded-xl shadow-lg border-2 border-white/20 active:scale-95 transition-all flex items-center gap-2"><Send size={14} /><span className="hidden xs:inline font-black text-[10px]">CANAL VIP</span></Button>
            <Button variant="ghost" onClick={onLogout} className="text-white p-1 hover:bg-red-500 rounded-full transition-colors"><LogOut size={20} /></Button>
          </div>
        </div>
      </header>

      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        <div className="bg-white border-b-2 border-slate-200 sticky top-[72px] md:top-[124px] z-40 shadow-sm">
