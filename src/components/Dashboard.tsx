...
export function Dashboard({ userRole, onLogout, tenantAgency }: any) {
  ...
  // --- REPARACIÓN BLINDADA PARA QUE EL BUILD NO FALLE ---
  useEffect(() => {
    let isMounted = true;
    const loadStats = async () => {
      try {
        ...
        // Verificación Senior: Si response es null, no hacemos nada y totalResults se queda en 0
      ...
      <main className="container mx-auto p-4">
        <Tabs value={activeTab} onValueChange={handleTabChange} className="space-y-4">
          ...
          <TabsContent value="admin" className="space-y-4">
            {isMasterAdmin ? (
              <><AdminAgencias /><div className="grid gap-4 lg:grid-cols-2"><AdminUserManagement /><AdminImageUpload /></div><DatoRicardo /><HistoryManager /></>
            ) : (
