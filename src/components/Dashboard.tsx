    ...
    <div className="min-h-screen bg-white text-slate-900 font-sans">
      <header className="sticky top-0 z-50 bg-slate-900 text-white border-b-8 border-emerald-500 px-4 py-4 shadow-2xl flex justify-between items-center">
        <div className="flex items-center gap-3">
           <img src="https://raw.githubusercontent.com/martvitalis1-ai/animalytics-lotto/main/src/assets/logo-animalytics.png" className="h-10" />
           <h1 className="font-black text-xl italic text-emerald-400 uppercase">ANIMALYTICS PRO</h1>
        </div>
        <div className="flex items-center gap-2">
          <a href="https://t.me/" target="_blank" className="bg-sky-500 px-4 py-2 rounded-full text-white font-black text-[10px] uppercase flex items-center gap-2 shadow-lg"><Send size={14} /> CANAL DE TELEGRAM</a>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          ...
          <div className="max-w-7xl mx-auto flex flex-col items-center gap-4">
            <div className="bg-white border-4 border-slate-900 rounded-2xl p-1 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] w-full max-w-sm">
              <Select value={globalLottery} onValueChange={setGlobalLottery}>
                ...
                <SelectContent className="bg-white border-2 border-slate-900">{LOTTERIES.map(l => (<SelectItem key={l.id} value={l.id} className="font-bold text-slate-900"><div className="flex items-center gap-2"><img src={getLotteryLogo(l.id)} className="w-4 h-4 rounded-full" /> {l.name}</div></SelectItem>))}</SelectContent>
              </Select>
            </div>
            <TabsList className="bg-white p-2 rounded-[2rem] border-4 border-slate-900 shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] grid grid-cols-4 h-auto w-full max-w-4xl">
              {["ia", "explosivo", "deportes", "resultados", "matriz", "guia", "agencias"].map((t) => (
                <TabsTrigger key={t} value={t} className="rounded-full px-2 py-2 font-black text-[9px] md:text-[11px] uppercase data-[state=active]:bg-emerald-600 data-[state=active]:text-white m-1">{t.toUpperCase()}</TabsTrigger>
              ))}
        ...
        <div className="p-4 max-w-7xl mx-auto pb-40">
          ...
          <TabsContent value="matriz" className="mt-0 space-y-12"><FrequencyHeatmap lotteryId={globalLottery} /><SequenceMatrixView lotteryId={globalLottery} /></TabsContent>
