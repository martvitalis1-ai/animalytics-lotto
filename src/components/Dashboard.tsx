...
export function Dashboard({ userRole, onLogout }: DashboardProps) {
  ...
  const [totalResults, setTotalResults] = useState<number>(4125); // Valor base real
  ...
  useEffect(() => {
    const loadTotalCount = async () => {
      try {
        ...
        if (!error && count) setTotalResults(count);
      } catch (e) {
        ...
        <div className="container mx-auto px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div className="hidden sm:block">
              ...
              <p className="text-xs text-muted-foreground">{totalResults.toLocaleString()}+ sorteos registrados</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            ...
            <span className={`px-2 py-1 rounded text-xs font-medium ${userRole === 'admin' ? 'bg-accent text-accent-foreground' : 'bg-muted text-muted-foreground'}`}>
      ...
      <main className="container mx-auto px-4 py-4">
        <Tabs value={activeTab} onValueChange={handleTabChange} className="space-y-4">
          <TabsList className="flex flex-wrap gap-1 h-auto p-1 bg-muted/50">
            <TabsTrigger value="ia"><Brain className="w-4 h-4 mr-1.5" />IA Predictiva</TabsTrigger>
