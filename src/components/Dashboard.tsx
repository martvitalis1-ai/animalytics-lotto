...
export function Dashboard({ userRole, onLogout, tenantAgency }: any) {
  ...
  // 🛡️ MAPEO PARA QUE CARGUEN RESULTADOS Y MATRIZ (EVITA PANTALLAS VACÍAS)
    ...
    <div className="min-h-screen bg-[#F8FAFC] text-slate-900 font-sans antialiased">
          ...
          <div className="flex items-center gap-2 flex-1 justify-end">
            <div className="bg-white rounded-xl p-0.5 border-2 border-emerald-500">
              <Select value={globalLottery} onValueChange={setGlobalLottery}>
                <SelectTrigger className="w-[125px] md:w-[220px] h-9 md:h-11 border-none bg-transparent font-black uppercase text-[10px] md:text-xs text-slate-900 px-2">
                  <SelectValue />
                </SelectTrigger>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        ...
        <div className="p-2 md:p-6 max-w-7xl mx-auto">
          {/* SECCIONES: PASAMOS dbId PARA QUE LOS RESULTADOS NO SALGAN VACÍOS */}
