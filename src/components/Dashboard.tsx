...
import { CombinationsView } from "./CombinationsView"; // 🛡️ Importado
...
export function Dashboard({ userRole, onLogout, tenantAgency }: any) {
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] text-slate-900 font-sans antialiased overflow-x-hidden">
      ...
      <header className="sticky top-0 z-[100] bg-slate-900 text-white border-b-4 border-emerald-500 px-6 md:px-10 py-2 md:py-4 shadow-2xl overflow-hidden">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-4">
          <div className="flex items-center shrink-0">
             <img src="https://raw.githubusercontent.com/martvitalis1-ai/animalytics-lotto/main/src/assets/logo-animalytics.png" className="h-16 md:h-28 w-auto object-contain drop-shadow-[0_0_20px_rgba(255,255,255,0.8)] ml-2 md:ml-4" alt="Logo" />
             <div className="flex flex-col ml-10 md:ml-16">
                ...
                <span className="text-[10px] md:text-[16px] font-black text-emerald-300 uppercase tracking-[0.35em] mt-2 drop-shadow-md">Bunker Intelligence</span>
             </div>
          </div>
          <div className="flex items-center gap-3 flex-1 justify-end">
            <div className="bg-white rounded-xl border-2 border-emerald-500 shadow-md flex-1 md:flex-none">
              <Select value={globalLottery} onValueChange={setGlobalLottery}>
                <SelectTrigger className="w-full md:w-[240px] h-11 border-none bg-transparent font-black uppercase text-[10px] md:text-sm text-slate-900 focus:ring-0 px-4"><SelectValue placeholder="Lotería" /></SelectTrigger>
                <SelectContent position="popper" sideOffset={5} className="border-2 border-slate-900 bg-white shadow-2xl z-[150]">
                  {LOTTERIES.map(l => (<SelectItem key={l.id} value={l.id} className="font-black text-slate-900 text-[10px] md:text-sm uppercase py-3">{l.name}</SelectItem>))}
                </SelectContent>
              </Select>
            </div>
            <Button onClick={() => window.open('https://t.me/+BXV4GahQ4gswNmNh', '_blank')} className="bg-[#229ED9] hover:bg-[#1e8ec5] h-11 px-4 rounded-xl shadow-lg border-2 border-white/20 active:scale-95 transition-all flex items-center justify-center gap-2">
              <Send size={18} className="fill-white" /><span className="hidden md:inline font-black text-xs">CANAL VIP</span>
            </Button>
            <Button variant="ghost" onClick={onLogout} className="text-white p-2 hover:bg-red-500 rounded-full transition-colors"><LogOut size={28} /></Button>
          </div>
        </div>
      </header>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        <div className="bg-white border-b-2 border-slate-200 sticky top-[84px] md:top-[124px] z-40 shadow-sm">
          <div className="max-w-7xl mx-auto p-1">
            {/* 🛡️ NAVEGACIÓN CORREGIDA CON "COMBINACIONES" */}
            <TabsList className="bg-transparent h-auto w-full grid grid-cols-4 md:flex md:justify-center p-1 gap-1.5">
              {["ia", "explosivo", "deportes", "resultados", "matriz", "combinaciones", "guia", "agencias"].map((t) => (
                <TabsTrigger key={t} value={t} className="px-1 py-2.5 font-black text-[9px] md:text-[12px] uppercase border-b-4 border-transparent data-[state=active]:border-emerald-500 data-[state=active]:text-emerald-600 rounded-none bg-transparent uppercase">{t}</TabsTrigger>
              ))}
        ...
        <div className="p-2 md:p-6 max-w-7xl mx-auto overflow-hidden">
          ...
          <TabsContent value="combinaciones" className="mt-0"><CombinationsView lotteryId={dbId} /></TabsContent>
