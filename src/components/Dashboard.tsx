      ...
      <header className="sticky top-0 z-[100] bg-slate-900 text-white border-b-4 border-emerald-500 px-4 md:px-10 py-2 md:py-4 shadow-2xl overflow-hidden">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-4">
          <div className="flex items-center w-full md:w-auto justify-between md:justify-start gap-4">
             ...
             <div className="flex md:hidden gap-2"><Button variant="ghost" onClick={onLogout} className="text-white p-1"><LogOut size={20} /></Button></div>
          </div>
          <div className="flex items-center gap-3 w-full md:w-auto justify-center md:justify-end">
            <div className="bg-white rounded-xl border-2 border-emerald-500 shadow-md flex-1 md:flex-none">
              <Select value={globalLottery} onValueChange={setGlobalLottery}>
                <SelectTrigger className="w-full md:w-[240px] h-10 border-none bg-transparent font-black uppercase text-[10px] md:text-xs text-slate-900 px-3"><SelectValue placeholder="Lotería" /></SelectTrigger>
            ...
            <Button onClick={() => window.open('https://t.me/+BXV4GahQ4gswNmNh', '_blank')} className="bg-[#229ED9] h-10 px-4 rounded-xl shadow-lg border-2 border-white/20 active:scale-95 transition-all flex items-center gap-2"><Send size={16} className="fill-white" /><span className="hidden xs:inline font-black text-[10px]">CANAL VIP</span></Button>
