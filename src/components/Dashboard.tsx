...
export function Dashboard({ userRole, onLogout, tenantAgency }: any) {
  ...
  // --- REPARACIÓN ATÓMICA: EVITA EL BUCLE Y EL ERROR 'COUNT' ---
  useEffect(() => {
    let isMounted = true;
    const loadCount = async () => {
      try {
        ...
        // Blindaje: Solo asignamos si la respuesta existe y no ha sido bloqueada
        if (isMounted && response && response.count !== null) {
          setTotalResults(Number(response.count));
        }
      } catch (e) {
        console.warn("Búnker esperando reconexión de red...");
      }
    };
    loadCount();
    return () => { isMounted = false; };
  }, []); // El array vacío es vital para que no entre en bucle
