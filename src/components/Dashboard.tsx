...
export function Dashboard({ userRole, onLogout, tenantAgency }: any) {
  ...
  // 🛡️ MAPEO DEFINITIVO PARA LA BASE DE DATOS (SEGÚN TU SQL)
  const getDbId = (id: string) => {
    const map: Record<string, string> = {
      'la_granjita': 'granjita',
      'el_guacharo': 'guacharo',
      'guacharo': 'guacharo',
      'guacharito': 'guacharito',
      'lotto_rey': 'lotto_rey',
      'selva_plus': 'selva_plus',
      'lotto_activo': 'lotto_activo'
    };
    return map[id] || id;
  };

  const dbId = getDbId(globalLottery);
    ...
    <div className="min-h-screen bg-[#F8FAFC] text-slate-900 font-sans antialiased">
        ...
        <div className="max-w-7xl mx-auto flex justify-between items-center gap-2">
          <div className="flex items-center gap-2 shrink-0">
             ...
             <div className="flex flex-col">
                ...
                <span className="text-[7px] md:text-[10px] font-bold text-emerald-500/60 uppercase tracking-widest ml-1 text-nowrap">Bunker Intelligence</span>
             </div>
          </div>
            ...
            <div className="bg-white rounded-xl border-2 border-emerald-500 shadow-md">
              <Select value={globalLottery} onValueChange={setGlobalLottery}>
                ...
                <SelectContent position="popper" sideOffset={5} className="border-2 border-slate-900 bg-white shadow-2xl z-[150]">
                  {LOTTERIES.map(l => (
                    <SelectItem key={l.id} value={l.id} className="font-black text-[10px] md:text-xs uppercase">{l.name}</SelectItem>
                  ))}
                </SelectContent>
              </Select>
            </div>
            <Button onClick={() => window.open('https://t.me/+BXV4GahQ4gswNmNh', '_blank')} className="bg-[#229ED9] h-10 px-3 rounded-xl shadow-lg"><Send size={14} /></Button>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        ...
        <div className="p-2 md:p-6 max-w-7xl mx-auto">
          {/* PASAMOS dbId PARA QUE TODO CARGUE */}
