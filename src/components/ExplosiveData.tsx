...
export function ExplosiveData({ lotteryId }: { lotteryId: string }) {
  ...
  useEffect(() => {
    async function load() {
      ...
      const { data } = await supabase.from('lottery_results').select('result_number').eq('lottery_type', lotteryId).limit(300);
      const freq: any = {};
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        {list.map((a, i) => (
          <div key={i} className="bg-white border-4 border-slate-900 rounded-[4rem] p-10 flex flex-col items-center shadow-[10px_10px_0px_0px_rgba(0,0,0,1)] relative">
            ...
            <h4 className="mt-6 font-black text-4xl italic">#{a.code}</h4>
          </div>
        ))}
      </div>
