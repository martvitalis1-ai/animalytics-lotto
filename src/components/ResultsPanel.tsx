  ...
  return (
    <div className="space-y-8 pb-40 px-2">
      <div className="bg-slate-900 text-white p-6 rounded-[3rem] border-b-8 border-emerald-500 shadow-2xl flex flex-col md:flex-row justify-between items-center gap-4">
        <h2 className="font-black text-xl md:text-2xl uppercase italic">Bóveda: {lotteryId.replace('_', ' ')}</h2>
      ...
      {loading ? <div className="p-20 text-center font-black animate-pulse text-emerald-500 uppercase">Abriendo Búnker...</div> : (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
          {times.map(time => {
            const res = results.find(r => r.draw_time.trim() === time.trim());
            return (
              <div key={time} className={`bg-white border-4 border-slate-900 rounded-[2.5rem] md:rounded-[3rem] p-6 flex flex-col items-center shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] relative min-h-[180px] md:min-h-[220px] ${res ? 'opacity-100' : 'opacity-10'}`}>
                <span className="absolute top-4 bg-slate-900 text-white px-4 py-1 rounded-full font-black text-[10px]">{time}</span>
                {res && <img src={getAnimalImageUrl(res.result_number)} className="w-32 h-32 md:w-48 md:h-48 mt-4 object-contain" />}
              </div>
            );
          })}
        </div>
