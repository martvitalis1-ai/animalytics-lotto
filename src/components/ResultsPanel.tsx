...
export function ResultsPanel({ lotteryId }: { lotteryId: string }) {
  ...
  useEffect(() => {
    async function fetchResults() {
      ...
      // 🛡️ BÚSQUEDA AGRESIVA: Ignora mayúsculas y espacios
      const cleanId = lotteryId.toLowerCase().trim();

      const { data, error } = await supabase
        .from('lottery_results')
        .select('*')
        .ilike('lottery_type', cleanId) // ilike es más potente que eq
        .eq('draw_date', selectedDate)
        .order('draw_time', { ascending: true });

      if (error) console.error("Error crítico en Resultados:", error);
  ...
  return (
    <div className="space-y-8 pb-40 px-2">
      <div className="bg-slate-900 text-white p-6 rounded-[3rem] border-b-8 border-emerald-500 shadow-2xl flex flex-col md:flex-row justify-between items-center gap-4">
        <h2 className="font-black text-xl md:text-2xl uppercase italic text-emerald-400">Bóveda: {lotteryId}</h2>
      ...
      {loading ? (
        <div className="p-20 text-center font-black animate-pulse text-slate-400 uppercase italic">Conectando con la Bóveda...</div>
      ) : results.length > 0 ? (
