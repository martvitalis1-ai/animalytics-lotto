...
export function ResultsPanel({ lotteryId }: { lotteryId: string }) {
  ...
  useEffect(() => {
    async function fetchResults() {
      setLoading(true);
      // dbId ya viene mapeado correctamente desde el Dashboard
    ...
    <div className="space-y-8 pb-40 px-1 animate-in fade-in duration-500">
          ...
          {times.map((slot) => {
            ...
            return (
              <div key={slot} className="bg-white border-4 border-slate-900 rounded-[2.5rem] md:rounded-[3rem] p-6 flex flex-col items-center shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] relative min-h-[180px]">
                ...
                <img src={getAnimalImageUrl(res.result_number)} className="w-32 h-32 md:w-44 md:h-44 mt-4 object-contain animate-in zoom-in" alt="Animal" />
              </div>
            );
          })}
        </div>
      ...
      {!loading && results.length === 0 && (
        <div className="p-20 text-center bg-white border-4 border-dashed border-slate-200 rounded-[3rem]">
          <p className="font-black text-slate-400 uppercase italic text-sm text-center">
            NO HAY REGISTROS PARA {lotteryId.toUpperCase()} EN LA FECHA {selectedDate}.
          </p>
        </div>
      )}
      <AdBanner slotId="ia" />
