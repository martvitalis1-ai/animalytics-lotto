    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        {drawTimes.map((time) => {
          ...
          return (
            <div key={time} className="relative p-6 rounded-[4rem] border-2 border-slate-900 bg-white flex flex-col items-center justify-center min-h-[320px] shadow-[6px_6px_0px_0px_rgba(15,23,42,1)]">
               ...
               {res ? (
                 <img src={getAnimalImageUrl(res.result_number)} className="w-48 h-48 md:w-56 md:h-56 object-contain mt-6 drop-shadow-2xl" alt="" />
               ) : (
                 <div className="w-32 h-32 flex items-center justify-center border-4 border-dashed border-slate-100 rounded-full italic text-slate-200 font-black uppercase text-[8px] text-center px-4">Esperando Sorteo</div>
               )}
            </div>
          );
        })}
