  ...
  return (
    <div className="space-y-8 pb-20">
      <div className="bg-slate-900 text-white p-6 rounded-[3rem] border-b-8 border-emerald-500 shadow-2xl flex justify-between items-center">
        <h2 className="font-black text-2xl uppercase italic">Bóveda</h2>
        <input type="date" value={selectedDate} onChange={(e) => setSelectedDate(e.target.value)}
          className="bg-white text-slate-900 px-4 py-2 rounded-xl font-black text-sm border-4 border-slate-900 shadow-lg outline-none"/>
      </div>
      {loading ? <div className="p-20 text-center font-black animate-pulse">ABRIENDO...</div> : (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          {times.map(time => {
            ...
            return (
              <div key={time} className={`bg-white border-4 border-slate-900 rounded-[3rem] p-6 flex flex-col items-center shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] relative min-h-[220px] transition-all ${res ? 'opacity-100' : 'opacity-20'}`}>
