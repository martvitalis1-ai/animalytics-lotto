  ...
  useEffect(() => {
    async function fetchResults() {
      const { data } = await supabase
        ...
        .select('*')
        .eq('lottery_type', lotteryId) // Sincronizado con granjita, lotto_rey, guacharito
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-700">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
        {times.map(time => {
          // 🛡️ Limpieza de datos para asegurar el match
          ...
          return (
            <div key={time} className={`bg-white border-4 border-slate-900 rounded-[3rem] p-6 flex flex-col items-center shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] relative min-h-[250px] transition-all ${res ? 'opacity-100' : 'opacity-30'}`}>
              ...
              {res ? (
                <img src={getAnimalImageUrl(res.result_number)} className="w-32 h-32 md:w-48 md:h-48 mt-4 object-contain drop-shadow-2xl" />
              ) : (
