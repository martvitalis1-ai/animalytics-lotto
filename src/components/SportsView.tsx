      ...
      {mode === 'deportes' && (
        <div className="space-y-8 relative z-10">
            ...
            <div className="bg-[#FFFCEB] border-4 border-slate-900 rounded-[3rem] p-6 md:p-10 shadow-[10px_10px_0px_0px_rgba(0,0,0,1)] space-y-8 animate-in zoom-in duration-500 text-slate-900 relative overflow-hidden">
               ...
               <div className="grid grid-cols-1 md:grid-cols-2 gap-6 relative z-10">
                  <div className="bg-white border-4 border-slate-900 p-6 rounded-[2.5rem] shadow-xl flex flex-col justify-between">
                     <div><span className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Juego Seleccionado</span><p className="font-black text-xl text-slate-900 uppercase italic leading-none mt-1">{matches[0].t1} vs {matches[0].t2}</p></div>
                     <div className="mt-6 bg-slate-900 p-4 rounded-2xl border-l-8 border-emerald-500"><span className="text-emerald-400 font-black text-[10px] uppercase italic">¿QUÉ APOSTAR?</span><p className="text-white font-black text-xl uppercase italic tracking-tighter">JUEGA A: {matches[0].gana}</p></div>
                  </div>
                  {matches[1] && (
                    <div className="bg-white border-4 border-slate-900 p-6 rounded-[2.5rem] shadow-xl flex flex-col justify-between">
                       <div><span className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Juego Seleccionado</span><p className="font-black text-xl text-slate-900 uppercase italic leading-none mt-1">{matches[1].t1} vs {matches[1].t2}</p></div>
                       <div className="mt-6 bg-slate-900 p-4 rounded-2xl border-l-8 border-orange-500"><span className="text-orange-400 font-black text-[10px] uppercase italic">¿QUÉ APOSTAR?</span><p className="text-white font-black text-xl uppercase italic tracking-tighter">JUEGA A: {matches[1].alta_baja}</p></div>
                    </div>
                  )}
               </div>
            </div>
          ...
          <div className="grid gap-10">
             {matches.map((m, i) => (
               <div key={i} className="bg-white border-4 border-slate-900 rounded-[3rem] md:rounded-[4rem] p-8 shadow-2xl relative text-slate-900 animate-in zoom-in duration-500">
          ...
          {/* 🛡️ BLOQUE NUEVO: GLOSARIO DE TÉRMINOS (SOLO EN DEPORTES) */}
          <div className="bg-white border-4 border-slate-900 rounded-[2.5rem] p-6 md:p-10 shadow-2xl space-y-6 text-slate-900">
             ...
             <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-3">
                   <div className="flex items-start gap-3"><HelpCircle className="text-slate-400 shrink-0 mt-1" size={16}/><p className="text-[11px] font-bold uppercase leading-tight"><span className="text-emerald-600">GANA (LOCAL/VISITANTE):</span> Apuestas directamente a quién ganará el encuentro.</p></div>
                   <div className="flex items-start gap-3"><HelpCircle className="text-slate-400 shrink-0 mt-1" size={16}/><p className="text-[11px] font-bold uppercase leading-tight"><span className="text-emerald-600">EMPATE:</span> El juego termina igualado (Común en fútbol).</p></div>
                   <div className="flex items-start gap-3"><HelpCircle className="text-slate-400 shrink-0 mt-1" size={16}/><p className="text-[11px] font-bold uppercase leading-tight"><span className="text-emerald-600">ALTA (OVER) / BAJA (UNDER):</span> Suma total de puntos o goles entre ambos equipos. Ejemplo: "Alta 2.5" requiere 3 o más goles.</p></div>
                </div>
                <div className="space-y-3">
                   <div className="flex items-start gap-3"><HelpCircle className="text-slate-400 shrink-0 mt-1" size={16}/><p className="text-[11px] font-bold uppercase leading-tight"><span className="text-emerald-600">RUNLINE (BÉISBOL):</span> Ventaja o desventaja de 1.5 carreras. "RL -1.5" obliga al equipo a ganar por 2 carreras o más.</p></div>
                   <div className="flex items-start gap-3"><HelpCircle className="text-slate-400 shrink-0 mt-1" size={16}/><p className="text-[11px] font-bold uppercase leading-tight"><span className="text-emerald-600">HÁNDICAP (BÁSQUET/NFL):</span> Ventaja de puntos dada a un equipo para equilibrar el juego.</p></div>
                   <p className="text-[9px] font-black text-slate-300 uppercase tracking-widest mt-4 italic">Animalytics Pro v4.0 • Inteligencia de Búnker</p>
                </div>
             </div>
          </div>
        </div>
