...
export function SportsView() {
  ...
  useEffect(() => {
    async function fetchCarteleraReal() {
      setLoading(true);
      // 🛡️ BUSCAMOS ABSOLUTAMENTE TODO LO QUE HAYA EN LA BASE DE DATOS PARA ESA FECHA
  ...
  const cuadroIA = useMemo(() => {
    return filteredRaces
      ...
      .map(r => ({
        leg: r.valida_text,
        picks: [r.favorite.match(/\(([^)]+)\)/)?.[1] || r.favorite], 
        tipo: "LÍNEA"
      }));
  }, [filteredRaces]);
  ...
  return (
    <div className="space-y-8 pb-40 px-1 animate-in fade-in duration-1000 relative min-h-screen">
      ...
      <div className="bg-slate-900 text-white p-8 rounded-[3rem] border-b-8 border-emerald-500 shadow-2xl relative z-10 text-left">
        ...
        <p className="text-emerald-400 font-black text-xs uppercase tracking-widest mt-2 ml-1">Sincronización Maestra • {date}</p>
      </div>
      ...
      {mode === 'deportes' && (
        <div className="space-y-8 relative z-10 text-slate-900 text-left">
           ...
           {showIA && filteredMatches.filter(m => m.is_ia_pick).length > 0 && (
             <div className="bg-[#FFFCEB] border-4 border-slate-900 rounded-[3rem] p-6 shadow-2xl space-y-6 animate-in zoom-in">
                ...
                {filteredMatches.filter(m => m.is_ia_pick).map((m, i) => (
                   <div key={i} className="bg-white border-4 border-slate-900 p-6 rounded-[2rem] shadow-xl mb-4">
           ...
           <div className="grid gap-10">
              {loading ? (
                 <div className="p-20 text-center font-black animate-pulse text-slate-300 uppercase">Sincronizando Cartelera...</div>
           ...
           <div className="bg-white border-4 border-slate-900 rounded-[2.5rem] p-6 md:p-10 shadow-2xl space-y-4 text-left">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-[11px] font-bold uppercase leading-tight">
                 ...
                 <p><span className="text-emerald-600">RUNLINE:</span> Ventaja 1.5 carreras.</p>
      ...
      {mode === 'hipismo' && (
        <div className="space-y-8 relative z-10 text-slate-900 text-left">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                   {cuadroIA.map((leg, i) => (
                     <div key={i} className="bg-white border-4 border-slate-900 p-4 rounded-[2rem] flex flex-col items-center">
                        ...
                        <span className="mt-2 px-3 py-0.5 rounded-full font-black text-[8px] uppercase border-2 bg-emerald-500 text-white border-emerald-700">LÍNEA</span>
                     </div>
                   ))}
                </div>
             </div>
           ...
           <div className="grid gap-10">
              {loading ? (
                 <div className="p-20 text-center font-black animate-pulse text-slate-300 uppercase">Cargando Programa...</div>
