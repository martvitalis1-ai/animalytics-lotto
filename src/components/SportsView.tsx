...
export function SportsView() {
  ...
  useEffect(() => {
    async function fetchCartelera() {
      ...
      // 1. Cargar Deportes del día seleccionado
      ...
      // 2. Cargar Hipismo del día seleccionado
  ...
  // Lógica de Filtrado por Deporte
  const filteredMatches = useMemo(() => sportsData.filter(j => j.sport === sport), [sportsData, sport]);
  const filteredRaces = useMemo(() => horseData.filter(r => r.track === track), [horseData, track]);

  // 🛡️ LÓGICA DE CUADRO IA (Armado automáticamente de los favoritos del día)
  const cuadroIA = useMemo(() => {
    return filteredRaces
      .filter(r => r.valida_text.includes('Válida') || r.valida_text.includes('Pick'))
      .slice(0, 6)
      .map(r => ({
        leg: r.valida_text,
        picks: [r.favorite.split('(')[1]?.replace(')','') || "01"],
        tipo: "LÍNEA"
      }));
  }, [filteredRaces]);
  ...
  return (
    <div className="space-y-8 pb-40 px-1 animate-in fade-in duration-1000 relative min-h-screen">
      ...
      {/* HEADER MAESTRO */}
      <div className="bg-slate-900 text-white p-8 rounded-[3rem] border-b-8 border-emerald-500 shadow-2xl relative overflow-hidden z-10 text-left">
        ...
        <div className="space-y-8 relative z-10 text-slate-900 text-left">
           ...
           {showIA && filteredMatches.filter(m => m.is_ia_pick).length > 0 && (
             <div className="bg-[#FFFCEB] border-4 border-slate-900 rounded-[3rem] p-6 shadow-2xl space-y-6 animate-in zoom-in">
                ...
                {filteredMatches.filter(m => m.is_ia_pick).slice(0,2).map((m, i) => (
                   <div key={i} className="bg-white border-4 border-slate-900 p-6 rounded-[2rem] shadow-xl">
              ...
              {filteredMatches.length > 0 ? filteredMatches.map((m, i) => (
                <div key={i} className="bg-white border-4 border-slate-900 rounded-[3rem] p-8 shadow-2xl relative">
                   ...
                   <div className="flex flex-col md:flex-row justify-between items-center gap-6 mb-8">
                      <p className="font-black text-2xl md:text-5xl uppercase text-slate-900">{m.team_home}</p>
                      ...
                      <p className="font-black text-2xl md:text-5xl uppercase text-slate-900">{m.team_away}</p>
                   </div>
                   <div className="grid grid-cols-3 gap-2">
                      <div className="bg-slate-900 p-2 rounded-xl text-center"><p className="text-[7px] text-emerald-400 font-black uppercase">Gana</p><p className="text-white font-black text-[10px]">{m.prediction_gana}</p></div>
                      <div className="bg-slate-900 p-2 rounded-xl text-center"><p className="text-[7px] text-emerald-400 font-black uppercase">Alta/Baja</p><p className="text-white font-black text-[10px]">{m.prediction_alta_baja}</p></div>
                      <div className="bg-slate-900 p-2 rounded-xl text-center"><p className="text-[7px] text-emerald-400 font-black uppercase">RL/H</p><p className="text-white font-black text-[10px]">{m.prediction_handicap}</p></div>
                   </div>
                </div>
              )) : <div className="py-20 text-center font-black text-slate-300 uppercase italic">No hay juegos automáticos cargados</div>}
           </div>
           ...
           <div className="bg-white border-4 border-slate-900 rounded-[2.5rem] p-6 shadow-2xl space-y-4 text-left">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-[11px] font-bold uppercase leading-tight">
                 <p><span className="text-emerald-600">GANA:</span> Apuestas al ganador.</p>
                 <p><span className="text-emerald-600">ALTA/BAJA:</span> Total de puntos/goles.</p>
           ...
           <div className="grid gap-10">
              {filteredRaces.length > 0 ? filteredRaces.map((r, i) => (
                <div key={i} className="bg-white border-4 border-slate-900 rounded-[3rem] p-8 shadow-2xl relative animate-in slide-in-from-bottom-6">
                   ...
                   <p className="mt-6 font-black text-xs text-slate-500 uppercase italic text-center">"{r.analysis}"</p>
                </div>
