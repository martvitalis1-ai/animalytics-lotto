...
import { getAnimalImageUrl, getAnimalName, getCodesForLottery } from '../lib/animalData';
...
export function FrequencyHeatmap({ lotteryId }: { lotteryId: string }) {
  ...
  useEffect(() => {
    async function load() {
      const { data: res } = await supabase.from('lottery_results').select('*').eq('lottery_type', lotteryId).limit(1000);
      setData(res || []);
    }
    load();
  }, [lotteryId]);
  ...
  return (
    <div className="bg-white border-4 border-slate-900 rounded-[4rem] p-10 shadow-xl overflow-hidden">
        ...
        <table className="w-full border-collapse border-2 border-slate-900">
          <thead>
            <tr className="bg-slate-900 text-white">
              ...
              {times.map(t => <th key={t} className="p-2 border border-slate-700 text-[9px] rotate-45 h-20">{t}</th>)}
            </tr>
          </thead>
          <tbody>
            {codes.map(code => (
              <tr key={code} className="hover:bg-slate-50 transition-colors">
                <td className="p-2 border border-slate-200 flex items-center gap-2">
                   ...
                   <span className="font-black text-xs">#{code}</span>
                </td>
                {times.map(t => {
                  const hits = data.filter(r => r.draw_time === t && r.result_number === code).length;
                  return (
                    <td key={t} className={`border border-slate-100 text-center font-black ${hits>0?'bg-emerald-500 text-white':''}`}>
                      {hits || '-'}
                    </td>
                  );
                })}
