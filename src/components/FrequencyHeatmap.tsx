...
export function FrequencyHeatmap({ lotteryId }: { lotteryId: string }) {
  ...
  useEffect(() => {
    async function load() {
      ...
      const { data: res } = await supabase.from('lottery_results').select('*').eq('lottery_type', dbId).limit(600);
      setData(res || []);
    }
    load();
  }, [lotteryId]);
        ...
        <table className="w-full border-collapse bg-white">
          <thead>
            <tr className="bg-slate-900 text-white">
              <th className="p-6 border-r border-slate-700 min-w-[160px]">ANIMAL</th>
            ...
            {codes.map(code => (
              <tr key={code} className="border-b-2 border-slate-100">
                <td className="p-4 border-r-4 border-slate-900 flex flex-col items-center justify-center bg-white sticky left-0 z-10">
                   {/* IMAGEN GIGANTE SIN TEXTO */}
                   <img src={getAnimalImageUrl(code)} className="w-28 h-28 md:w-36 md:h-36 object-contain drop-shadow-md" />
                </td>
                {times.map(t => {
                  const hits = data.filter(r => r.draw_time.trim() === t.trim() && r.result_number.trim() === code.trim()).length;
                  return (
                    <td key={t} className={`border-r border-slate-200 text-center font-black text-3xl ${getColor(hits)}`}>
                      {hits || '-'}
                    </td>
                  );
                })}
