...
export function FrequencyHeatmap() {
  ...
  const getCellColor = (count: number) => {
    ...
    if (count >= 3) return 'bg-red-500 text-white font-bold';
      ...
      <CardHeader className="pb-2 bg-muted/10 border-b">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button onClick={loadHeatData} variant="outline" size="icon" className="h-9 w-9">
      ...
      <CardContent className="p-0">
        <ScrollArea className="w-full h-[500px]">
          <table className="w-full border-collapse">
            <thead className="sticky top-0 z-20 bg-card border-b-2">
              <tr>
                ...
                {drawTimes.map(t => (
                  <th key={t} className="p-2 font-black text-[10px] min-w-[60px] text-center">{t.replace(':00 ', '')}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {numberRange.map(num => {
                ...
                return (
                  <tr key={num} className="hover:bg-primary/5 border-b">
                    <td className="p-3 sticky left-0 bg-card z-10 border-r-2 font-mono font-black">{num.padStart(2, '0')}</td>
                    ...
                    {drawTimes.map(t => {
                      ...
                      return (
                        <td key={t} className={`p-1 border text-center text-xs transition-all ${getCellColor(count)}`}>
                          {count > 0 ? count : '-'}
                        </td>
                      );
                    })}
