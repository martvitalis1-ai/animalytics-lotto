...
export function FrequencyHeatmap({ lotteryId }: { lotteryId: string }) {
  ...
  const freqMap = useMemo(() => {
    ...
    data.forEach(r => {
      ...
      map[`${tKey}-${nKey}`] = (map[`${tKey}-${nKey}`] || 0) + 1;
    });
    return map;
  }, [data]);
        ...
        <table className="w-full border-collapse bg-white table-auto">
          <thead>
            <tr className="bg-slate-900 text-white">
              <th className="p-2 border-r-4 border-slate-700 w-24 min-w-[90px] md:w-28 sticky left-0 bg-slate-900 z-20 font-black text-[10px] uppercase">Animal</th>
              {times.map(t => (
                <th key={t} className="p-0 border-r border-slate-700 min-w-[75px] md:min-w-[85px] h-24 relative overflow-hidden">
                  <div className="absolute inset-0 flex items-center justify-center">
                    <span className="transform -rotate-90 md:-rotate-45 font-black text-[10px] md:text-xs whitespace-nowrap tracking-tighter">{t}</span>
                  </div>
                </th>
              ))}
            </tr>
            ...
            {codes.map(code => (
              <tr key={code} className="border-b border-slate-100">
                <td className="p-2 border-r-4 border-slate-900 flex justify-center bg-white sticky left-0 z-10 shadow-lg">
                   <img src={getAnimalImageUrl(code)} className="w-14 h-14 md:w-20 md:h-20 object-contain" alt="" />
                </td>
                {times.map(t => {
                  const hits = freqMap[`${t.toString().replace(/\s/g, '').toUpperCase()}-${code.trim().padStart(2, '0').replace('000', '00')}`] || 0;
                  return (
                    <td key={t} className={`border-2 text-center font-black text-xl md:text-3xl transition-all ${getCellStyle(hits)}`}>
                      {hits || '-'}
                    </td>
                  );
                })}
