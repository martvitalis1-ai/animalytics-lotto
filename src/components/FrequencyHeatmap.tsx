...
export function FrequencyHeatmap({ lotteryId }: { lotteryId: string }) {
  ...
  return (
    <div className="bg-white border-4 border-slate-900 rounded-[4rem] p-6 shadow-2xl overflow-hidden">
      <h3 className="font-black text-2xl uppercase italic mb-6 flex items-center gap-2"><Grid3X3 className="text-emerald-500" /> Matriz de Frecuencia Atómica</h3>
          ...
          <tbody>
            {codes.map(code => (
              <tr key={code} className="hover:bg-slate-50">
                <td className="p-2 border border-slate-200 flex items-center gap-2"><img src={getAnimalImageUrl(code)} className="w-10 h-10" /><span className="font-black text-[10px]">#{code}</span></td>
                {times.map(t => {
