...
export function FrequencyHeatmap({ lotteryId }: { lotteryId: string }) {
  ...
  useEffect(() => {
    async function load() {
      // Limitamos a 400 para velocidad y evitar error 429
  ...
  return (
    <div className="bg-white border-4 border-slate-900 rounded-[4rem] p-10 shadow-2xl overflow-hidden">
      <h3 className="font-black text-3xl uppercase italic mb-8 flex items-center gap-4 text-slate-900"><Grid3X3 className="text-emerald-500" /> Matriz Atómica por Sorteo</h3>
        ...
        <table className="w-full border-collapse border-2 border-slate-900 text-slate-900">
          ...
          <tbody>{codes.map(code => (
              <tr key={code} className="hover:bg-slate-50 transition-all border-b">
                <td className="p-2 border-r flex items-center gap-2"><img src={getAnimalImageUrl(code)} className="w-10 h-10 object-contain" /><span className="font-black text-[10px]">#{code}</span></td>
                {times.map(t => {
