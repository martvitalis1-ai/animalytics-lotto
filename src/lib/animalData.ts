// ============================================================
// ANIMAL DATA - VERSIÓN ULTRA-BLINDADA v106.7
// ============================================================
...
// RUTA PURA (Carga%20de%20Animales con A mayúscula)
const BASE_URL = "https://raw.githubusercontent.com/martvitalis1-ai/animalytics-lotto/main/src/assets/Carga%20de%20Animales";

export const getAnimalImageUrl = (code: string | number): string => {
  const strCode = String(code).trim();
  // Blindaje: '0' y '00' pasan directo, 1-9 se les pone el 0 (01, 02...)
  const normalized = (strCode === '0' || strCode === '00') ? strCode : strCode.padStart(2, '0');
  return `${BASE_URL}/${normalized}.png`;
};
...
export const getAnimalByCode = (code: string | number) => {
  ...
  const strCode = String(code).trim();
  const normalized = (strCode === '0' || strCode === '00') ? strCode : strCode.padStart(2, '0');
  return { id: parseInt(normalized) || 0, code: normalized, name: getAnimalName(normalized), category: "general" };
};
